...
//! ProgramInfo extension for Haneul Flavor
//! Contains information that may be expensive to compute and is needed only for Haneul
...
pub enum TransferKind {
    ...
    PublicTransfer(Loc),
    /// transferred within the module to an address vis `haneul::transfer::transfer`
    PrivateTransfer(Loc),
}

/// Haneul info for the entire program (mostly used during translation ).
#[derive(Debug, Clone)]
pub struct HaneulInfo {
...
/// Same as `HaneulInfo` but for a given module (store per-module in `ProgramInfo`).
#[derive(Debug, Clone)]
pub struct HaneulModInfo {
    ...
    fn visit_ty(
        ...
        match ty_.inner() {
            ...
            TI::Apply(_, sp!(_, tn_), _)
                if tn_.is(&HANEUL_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME) =>
            {
