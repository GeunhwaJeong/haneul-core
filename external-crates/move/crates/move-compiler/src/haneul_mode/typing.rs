...
impl<'a> Context<'a> {
    fn new(env: &'a CompilationEnv, info: Arc<TypingProgramInfo>) -> Self {
        let haneul_module_ident = info
            .modules
            .key_cloned_iter()
            .find(|(m, _)| m.value.is(&HANEUL_ADDR_VALUE, TRANSFER_MODULE_NAME))
            .map(|(m, _)| m);
        ...
        Context {
            ...
            reporter,
            info,
            haneul_transfer_ident: haneul_module_ident,
            current_module: None,
            otw_name: None,
...
fn struct_def(context: &mut Context, name: DatatypeName, sdef: &N::StructDefinition) {
    ...
    if !id_field_type
        .value
        .is(&HANEUL_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME)
    {
        let actual = format!(
            "But found type: {}",
...
fn invalid_object_id_field_diag(key_loc: Loc, loc: Loc, name: DatatypeName) -> Diagnostic {
    const KEY_MSG: &str = "The 'key' ability is used to declare objects in Haneul";
...
fn exp(context: &mut Context, e: &T::Exp) {
    match &e.exp.value {
        T::UnannotatedExp_::ModuleCall(mcall) => {
            ...
            if module.value.is(&HANEUL_ADDR_VALUE, EVENT_MODULE_NAME)
            ...
            let is_transfer_module = module.value.is(&HANEUL_ADDR_VALUE, TRANSFER_MODULE_NAME);
...
fn otw_special_cases(context: &Context) -> bool {
    BRIDGE_SUPPORTED_ASSET
        ...
        || context
            .current_module()
            .value
            .is(&HANEUL_ADDR_VALUE, HANEUL_MODULE_NAME)
}
...
fn check_private_transfer(context: &mut Context, loc: Loc, mcall: &ModuleCall) {
    ...
    if current_module
        .value
        .is(&HANEUL_ADDR_VALUE, TRANSFER_FUNCTION_NAME)
    {
