...
fn test_config(path: &Path) -> (TestKind, TestInfo, PackageConfig, Flags) {
    ...
    let flavor = if path_contains(HANEUL_MODE_DIR) {
        Flavor::Haneul
    } else {
        Flavor::default()
    };
...
pub fn run_test(path: &Path) -> datatest_stable::Result<()> {
    ...
    if flavor == Flavor::Haneul {
        let (prefix, filters) = haneul_mode::linters::known_filters();
        compiler = compiler.add_custom_known_filters(prefix, filters);
        if test_info.lint {
            compiler = compiler.add_visitors(haneul_mode::linters::linter_visitors(LintLevel::All))
        }
    }
