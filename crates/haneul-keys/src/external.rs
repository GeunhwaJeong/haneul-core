...
pub struct External {
    ...
    pub aliases: BTreeMap<HaneulAddress, Alias>,
    // Holds a map of addresses to [`StoredKey`]
    pub keys: BTreeMap<HaneulAddress, StoredKey>,
...
pub struct StoredKey {
    ...
    /// External signer binary (e.g., "haneul-ledger-signer")
...
impl External {
    ...
    pub fn load_or_create(path: &PathBuf) -> Result<Self, Error> {
        ...
        let aliases: BTreeMap<HaneulAddress, Alias> = if aliases_store_directory.exists() {
        ...
        let keys: BTreeMap<HaneulAddress, StoredKey> = if path.exists() {
    ...
    /// Add a Key ID from the given an external signer to the Haneul CLI index
...
impl AccountKeystore for External {
    ...
    async fn generate(
        ...
        let address: HaneulAddress = (&stored_key.public_key).into();
    ...
    async fn remove(&mut self, address: HaneulAddress) -> Result<(), Error> {
    ...
    /// Export the key pair for the given address as a `HaneulKeyPair`.
    fn export(&self, _address: &HaneulAddress) -> Result<&HaneulKeyPair, Error> {
    ...
    async fn sign_hashed(
        ...
        let address = HaneulAddress::from(&public_key);
        match &result.signature {
            Signature::Ed25519HaneulSignature(s) => {
                ...
                let signature_address = HaneulAddress::from(&pk);
            ...
            Signature::Secp256k1HaneulSignature(s) => {
                let (sig, pk) = s.get_verification_inputs().map_err(|e| {
                    signature::Error::from_source(anyhow!(
                        "Failed to get verification inputs: {}",
                        e
            ...
            Signature::Secp256r1HaneulSignature(s) => {
                let (sig, pk) = s.get_verification_inputs().map_err(|e| {
                    signature::Error::from_source(anyhow!(
                        "Failed to get verification inputs: {}",
                        e
    ...
    fn addresses_with_alias(&self) -> Vec<(&HaneulAddress, &Alias)> {
    ...
    fn get_alias(&self, address: &HaneulAddress) -> Result<String, anyhow::Error> {
...
mod tests {
    ...
    async fn test_serialize() {
        ...
        external.keys.insert(
            HaneulAddress::from_str(ADDRESS).unwrap(),
            StoredKey {
    ...
    async fn test_exec() {
        ...
        assert!(
            external
                .exec("haneul-key-tool", "test_method", params)
                .await
                .is_ok()
        );
    }
    ...
    async fn test_generate_success() {
        ...
        assert_eq!(address, HaneulAddress::from_str(ADDRESS).unwrap());
    ...
    async fn test_add_existing_key() {
        ...
        let key = keys.get(&HaneulAddress::from_str(ADDRESS).expect("Invalid address format"));
        assert!(key.is_some());

        let alias = external
            .aliases
            .get(&HaneulAddress::from_str(ADDRESS).expect("Invalid address format"));
    ...
    async fn test_add_keypair_not_supported() {
        ...
        let result = external
            .import(None, HaneulKeyPair::Ed25519(ed25519_keypair))
            .await;
    ...
    async fn test_remove_key() {
        ...
        let address = HaneulAddress::from_str(ADDRESS).unwrap();
        external.keys.insert(
            address,
            StoredKey {
                public_key: PublicKey::decode_base64(PUBLIC_KEY).unwrap(),
    ...
    fn test_get_key_fails() {
        ...
        let result = external.export(&HaneulAddress::from_str(ADDRESS).unwrap());
        assert!(result.is_err())
    }
