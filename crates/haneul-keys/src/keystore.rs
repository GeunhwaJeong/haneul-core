...
pub trait AccountKeystore: Send + Sync {
    ...
    /// Import a keypair into the keystore from a `HaneulKeyPair` and optional alias.
    async fn import(
        &mut self,
        alias: Option<String>,
    ...
    async fn remove(&mut self, address: HaneulAddress) -> Result<(), anyhow::Error>;
    /// Return an array of `PublicKey`, consisting of every public key in the keystore.
    fn entries(&self) -> Vec<PublicKey>;
    /// Return `HaneulKeyPair` for the given address.
    fn export(&self, address: &HaneulAddress) -> Result<&HaneulKeyPair, anyhow::Error>;
    ...
    /// Return an array of `HaneulAddress`, consisting of every address in the keystore.
    fn addresses(&self) -> Vec<HaneulAddress> {
        self.entries().iter().map(|k| k.into()).collect()
    }
    /// Return an array of (&HaneulAddress, &Alias), consisting of every address and its corresponding alias in the keystore.
    ...
    fn get_by_identity(&self, key_identity: &KeyIdentity) -> Result<HaneulAddress, anyhow::Error> {
        match key_identity {
...
pub struct FileBasedKeystore {
    ...
    aliases: BTreeMap<HaneulAddress, Alias>,
    path: Option<PathBuf>,
}
        ...
        let aliases = if aliases_path.exists() {
            ...
            aliases
                ...
                .map(|alias| {
                    ...
                    key.map(|k| (Into::<HaneulAddress>::into(&k), alias))
                })
                .collect::<Result<BTreeMap<_, _>, _>>()
                .map_err(|e| {
                    anyhow!(
...
impl InMemKeystore {
    pub fn new_insecure_for_tests(initial_key_number: usize) -> Self {
        ...
        let keys = (0..initial_key_number)
            ...
            .map(|(ad, k)| (ad, HaneulKeyPair::Ed25519(k)))
...
mod tests {
    ...
    fn validate_alias_test() {
        ...
        assert!(validate_alias("abc_123.haneul").is_ok());
        // Not allowed
