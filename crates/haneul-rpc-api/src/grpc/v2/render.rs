...
use haneul_rpc::{
    field::FieldMaskTree,
    merge::Merge,
    proto::haneul::rpc::v2::{Bcs, Event, Object, TransactionEffects, TransactionEvents},
};
...
impl RpcService {
    ...
    pub fn render_json(
        ...
        haneul_types::proto_value::ProtoVisitor::new(self.config.max_json_move_value_size())
    ...
    pub fn render_events_to_proto(
        &self,
        events: &haneul_types::effects::TransactionEvents,
        mask: &FieldMaskTree,
    ) -> TransactionEvents {
    ...
    pub fn render_effects_to_proto(
        &self,
        effects: &haneul_types::effects::TransactionEffects,
        unchanged_loaded_runtime_objects: &[haneul_types::storage::ObjectKey],
        objects: &haneul_types::full_checkpoint_content::ObjectSet,
        mask: &FieldMaskTree,
    ) -> TransactionEffects {
        ...
        if mask.contains(TransactionEffects::CHANGED_OBJECTS_FIELD) {
            for changed_object in effects.changed_objects.iter_mut() {
                let Ok(object_id) = changed_object
                    .object_id()
                    .parse::<haneul_types::base_types::ObjectID>()
                else {
                    continue;
                };

                if let Some(object) = objects.get(&haneul_types::storage::ObjectKey(
                    object_id,
                    changed_object
                        .input_version_opt()
