...
impl Client {
    ...
    pub fn inner_mut(&mut self) -> &mut haneul_rpc::Client {
        &mut self.0
    }
    ...
    pub async fn get_committee(
        &self,
        epoch: Option<u64>,
    ) -> Result<haneul_types::committee::Committee> {
        let mut request = proto::GetEpochRequest::default();
        if let Some(epoch) = epoch {
            request.set_epoch(epoch);
        }
    ...
    pub async fn list_delegated_stake(
        &self,
        owner: HaneulAddress,
    ) -> Result<Vec<haneul_rpc::client::DelegatedStake>> {
        self.0.clone().list_delegated_stake(&owner.into()).await
    }

    pub fn transaction_builder(&self) -> haneul_transaction_builder::TransactionBuilder {
        haneul_transaction_builder::TransactionBuilder::new(std::sync::Arc::new(self.clone()) as _)
    }
}
...
impl ExecutedTransaction {
    ...
    pub fn get_new_package_obj(&self) -> Option<haneul_types::base_types::ObjectRef> {
    ...
    pub fn get_new_package_upgrade_cap(&self) -> Option<haneul_types::base_types::ObjectRef> {
        use haneul_rpc::proto::haneul::rpc::v2::changed_object::OutputObjectState;
    ...
    pub fn timestamp_ms(&self) -> Option<u64> {
        self.timestamp
            .and_then(|timestamp| haneul_rpc::proto::proto_to_timestamp_ms(timestamp).ok())
    }
}
...
fn certified_checkpoint_summary_try_from_proto(
    ...
    let signature = haneul_types::crypto::AuthorityStrongQuorumSignInfo::from(
        haneul_sdk_types::ValidatorAggregatedSignature::try_from(
            checkpoint
                .signature
                .as_ref()
...
impl haneul_transaction_builder::DataReader for Client {
    async fn get_owned_objects(
        &self,
        address: HaneulAddress,
        object_type: move_core_types::language_storage::StructTag,
    ) -> Result<Vec<haneul_types::base_types::ObjectInfo>, anyhow::Error> {
        self.list_owned_objects(address, Some(object_type))
            .map_ok(|o| haneul_types::base_types::ObjectInfo::from_object(&o))
            .try_collect()
            .await
            .map_err(Into::into)
    }
