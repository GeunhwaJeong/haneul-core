...
use haneul_package_management::system_package_versions::{SYSTEM_GIT_REPO, latest_system_packages};
...
impl HaneulFlavor {
    ...
    fn system_deps_by_name() -> BTreeMap<String, SystemDepName> {
        BTreeMap::from([
            ("Haneul".into(), "haneul".into()),
            ("HaneulSystem".into(), "haneul_system".into()),
...
impl MoveFlavor for HaneulFlavor {
    ...
    fn implicit_dependencies(_: &EnvironmentID) -> BTreeMap<PackageName, ReplacementDependency> {
        BTreeMap::from([
            (
                PackageName::new("haneul").expect("haneul is a valid identifier"),
                ReplacementDependency::override_system_dep("haneul"),
            ),
            (
...
fn validate_modern_manifest_does_not_use_legacy_system_names(
    ...
    let legacy_names = HaneulFlavor::system_deps_by_name();
