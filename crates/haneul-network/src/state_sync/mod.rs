...
mod generated {
    include!(concat!(env!("OUT_DIR"), "/haneul.StateSync.rs"));
}
mod builder;
mod metrics;
mod server;
...
use haneul_data_ingestion_core::{ReaderOptions, setup_single_workflow_with_options};
    ...
    S: WriteStore + Clone + Send + Sync + 'static,
{
    ...
    if sync_from_archive {
        ...
        if ingestion_url.contains("checkpoints.mainnet.haneul.io") {
