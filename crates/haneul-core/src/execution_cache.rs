...
/// Should only be used for haneul-tool or tests. Nodes must use build_execution_cache which
...
pub trait ObjectCacheRead: Send + Sync {
    fn get_package_object(&self, id: &ObjectID) -> HaneulResult<Option<PackageObject>>;
...
pub trait TransactionCacheRead: Send + Sync {
    ...
    fn get_transactions_and_serialized_sizes(
        &self,
        digests: &[TransactionDigest],
    ) -> HaneulResult<Vec<Option<(VerifiedTransaction, usize)>>> {
    ...
    fn notify_read_executed_effects_may_fail<'a>(
        ...
        task_name: &'static str,
        digests: &'a [TransactionDigest],
    ) -> BoxFuture<'a, HaneulResult<Vec<TransactionEffects>>> {
        async move {
    ...
    ($implementor: ident) => {
        ...
        impl BackingPackageStore for $implementor {
            fn get_package_object(
                &self,
                package_id: &ObjectID,
            ) -> HaneulResult<Option<PackageObject>> {
...
macro_rules! implement_passthrough_traits {
    ($implementor: ident) => {
        ...
        impl ExecutionCacheReconfigAPI for $implementor {
            ...
            fn checkpoint_db(&self, path: &std::path::Path) -> HaneulResult {
