...
impl From<BalanceIndexInfo> for haneul_types::storage::BalanceInfo {
    fn from(index_info: BalanceIndexInfo) -> Self {
        ...
        // Anyone can call `haneul::balance::create_supply` and mint unbounded supply, potentially pushing
        ...
        haneul_types::storage::BalanceInfo { balance }
    }
}
...
impl IndexStoreTables {
    ...
    fn extract_accumulator_version(
        &self,
        tx: &haneul_types::full_checkpoint_content::CheckpointTransaction,
    ) -> Option<u64> {
    ...
    fn index_transactions(
        ...
        for (tx_idx, tx) in checkpoint.transactions.iter().enumerate().rev() {
            let balance_changes = haneul_types::balance_change::derive_balance_changes(
                &tx.effects,
                &tx.input_objects,
                &tx.output_objects,
            )
    ...
    fn index_object(&mut self, object: Object) -> Result<(), StorageError> {
        match object.owner {
            ...
            Owner::ObjectOwner(parent) => {
                ...
                if parent == HANEUL_ACCUMULATOR_ROOT_OBJECT_ID.into()
...
fn get_address_balance_info(object: &Object) -> Option<(HaneulAddress, StructTag, i128)> {
    ...
    let (key, value): (
        ...
        haneul_types::accumulator_root::AccumulatorValue,
