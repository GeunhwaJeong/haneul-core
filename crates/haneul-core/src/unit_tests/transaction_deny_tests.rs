...
type Account = (HaneulAddress, Ed25519KeyPair, Vec<ObjectRef>);
...
fn get_accounts_and_coins(
    ...
    let accounts: Vec<_> = network_config
        ...
        .iter()
        .map(|account| {
            let address: HaneulAddress = account.public().into();
...
fn process_zklogin_tx(tx: Transaction, state: &Arc<AuthorityState>) -> HaneulResult<()> {
...
fn transfer_with_account(
    ...
    let data = TransactionData::new_transfer_haneul_allow_sponsor(
        sender_account.0,
        sender_account.0,
        None,
        sponsor_account.2[0],
