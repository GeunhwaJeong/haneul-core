...
use haneul_json_rpc_types::{
    ...
    HaneulTransactionBlockEffectsV1, HaneulTypeTag,
};
...
use haneul_types::{HANEUL_CLOCK_OBJECT_SHARED_VERSION, digests::Digest};
use haneul_types::{dynamic_field::DynamicFieldType, messages_consensus::ConsensusTransaction};
...
async fn test_dry_run_no_gas_big_transfer() {
    ...
    builder.transfer_haneul(recipient, Some(amount));
...
async fn test_dev_inspect_gas_coin_argument() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_haneul(vec![recipient], vec![amount]).unwrap();
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    ...
    assert_eq!(arg, &HaneulArgument::GasCoin);
    check_coin_value(
        arg_value,
        arg_type,
...
fn check_coin_value(actual_value: &[u8], actual_type: &HaneulTypeTag, expected_value: u64) {
...
#[tokio::test]
async fn test_handle_transfer_haneul_with_amount_insufficient_gas() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let object_id = ObjectID::random();
    let authority_state = init_state_with_ids(vec![(sender, object_id)]).await;
...
#[tokio::test]
async fn test_genesis_haneul_system_state_object() {
    ...
    assert!(move_object.type_().is(&HaneulSystemStateWrapper::type_()));
...
#[tokio::test]
async fn test_transfer_haneul_no_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
    let init_balance = haneul_types::gas::get_gas_balance(&gas_object).unwrap();
...
#[tokio::test]
async fn test_transfer_haneul_with_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
...
#[tokio::test]
async fn test_clear_cache_reverts_transfer_haneul() {
...
async fn test_gas_smashing() {
    ...
    fn make_gas_coins(owner: HaneulAddress, gas_amount: u64, coin_num: u64) -> Vec<Object> {
        let mut objects = vec![];
    ...
    async fn run_and_check(
        ...
        let balance =
            haneul_types::gas::get_gas_balance(&state.get_object(&gas_coin_ids[0]).await.unwrap())
                .unwrap();
...
async fn test_publish_not_a_package_dependency() {
    ...
    deps.push(HANEUL_SYSTEM_STATE_OBJECT_ID);
    builder.publish_immutable(modules, deps);
    let kind = TransactionKind::programmable(builder.finish());
...
pub fn create_gas_objects(num: u32, owner: HaneulAddress) -> Vec<Object> {
    let mut objects = vec![];
    for _ in 0..num {
        let gas_object_id = ObjectID::random();
