...
use haneul_types::executable_transaction::VerifiedExecutableTransactionWithAliases;
...
impl SharedObjVerManager {
    ...
    ) -> HaneulResult<ConsensusSharedObjVerAssignment>
    where
        T: AsTx + 'a,
    {
    ...
    pub fn assign_versions_for_certificate(
        ...
        let accumulator_version = if epoch_store.accumulators_enabled() {
            ...
            let accumulator_version = *shared_input_next_versions
                .get(&(HANEUL_ACCUMULATOR_ROOT_OBJECT_ID, accumulator_initial_version))
            ...
            } in shared_input_objects.iter()
            {
                let assigned_version = match cancellation_info {
                    ...
                    Some(CancelConsensusCertificateReason::DkgFailed) => {
                        if id == &HANEUL_RANDOMNESS_STATE_OBJECT_ID {
...
mod tests {
    ...
    use haneul_types::{HANEUL_ACCUMULATOR_ROOT_OBJECT_ID, HANEUL_RANDOMNESS_STATE_OBJECT_ID};
    ...
    async fn test_assign_versions_from_consensus_with_randomness() {
        ...
        assert_eq!(
            epoch_store
                .get_next_object_version(&HANEUL_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version)
                .unwrap(),
            randomness_obj_version
        );
        ...
        assert_eq!(
            *shared_input_next_versions
                .get(&(HANEUL_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version))
                .unwrap(),
        ...
        assert_eq!(
            ...
            vec![
                (
                    ...
                    AssignedVersions::new(
                        vec![(
                            (HANEUL_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version),
                            randomness_obj_version
                        )],
                        Some(expected_accumulator_version)
                    )
            ...
            ConsensusSharedObjVerAssignment {
                assigned_versions: AssignedTxAndVersions::new(vec![
                    ...
                    (
                        ...
                        AssignedVersions {
                            shared_object_versions: vec![(
                                (HANEUL_ACCUMULATOR_ROOT_OBJECT_ID, acc_version),
                                acc_version
                            )],
                            accumulator_version: Some(acc_version),
                        }
