...
use haneul_types::{effects::TransactionEvents, messages_grpc::SubmitTxType};
...
use haneul_config::local_ip_utils::new_local_tcp_address_for_testing;
use haneul_types::messages_grpc::PingType;
...
impl ValidatorService {
    ...
    pub fn handle_transaction_for_testing(&self, transaction: Transaction) -> HaneulResult<()> {
    ...
    async fn collect_immutable_object_ids(
        ...
        objects
            ...
            .filter_map(|(obj, id)| {
                let Some(o) = obj else {
                    return Some(Err::<ObjectID, HaneulError>(
            ...
            .collect::<HaneulResult<Vec<ObjectID>>>()
    }
    ...
    async fn handle_submit_transaction(
        ...
        loop {
            ...
            match res {
                ...
                Err(err) => {
                    if max_retries > 0
                        && let HaneulErrorKind::ValidatorHaltedAtEpochEnd = err.as_inner()
                    {
                        max_retries -= 1;
    ...
    async fn handle_submit_transaction_inner(
        ...
        if is_ping_request {
            fp_ensure!(
                ...
                HaneulErrorKind::InvalidRequest(format!(
        ...
        for (idx, tx_bytes) in request.transactions.iter().enumerate() {
            let transaction = match bcs::from_bytes::<Transaction>(tx_bytes) {
                ...
                Err(e) => {
                    ...
                    return Err(HaneulErrorKind::TransactionDeserializationError {
    ...
    fn try_from_submit_tx_response(
        ...
        for (i, result) in results.into_iter().enumerate() {
            let result = result.ok_or_else(|| HaneulErrorKind::GenericAuthorityError {
    ...
    ) -> HaneulResult<(Option<TransactionEvents>, Vec<Object>, Vec<Object>)> {
...
impl ValidatorService {
    ...
    async fn wait_for_effects_impl(
        ...
        let response = timeout(
            ...
            epoch_store
                ...
                .map_err(|_| HaneulErrorKind::EpochEnded(epoch_store.epoch())),
        )
        .await
    ...
    async fn wait_for_effects_response(
        ...
        epoch_store: &Arc<AuthorityPerEpochStore>,
    ) -> HaneulResult<WaitForEffectsResponse> {
        if request.ping_type.is_some() {
            ...
            .await
            .map_err(|_| HaneulErrorKind::TimeoutError)?;
        }
    ...
    ) -> WrappedServiceResponse<HaneulSystemState> {
        let response = self
            .state
    ...
    async fn validator_health_impl(
        &self,
        _request: tonic::Request<haneul_types::messages_grpc::RawValidatorHealthRequest>,
    ) -> WrappedServiceResponse<haneul_types::messages_grpc::RawValidatorHealthResponse> {
        ...
        if let Some(traffic_controller) = &self.traffic_controller {
            if !traffic_controller.check(&client, &None).await {
                ...
                Err(tonic::Status::from_error(
                    HaneulErrorKind::TooManyRequests.into(),
                ))
            } else {
                Ok(())
            }
    ...
    fn handle_traffic_resp<T>(
        ...
        let (error, spam_weight, unwrapped_response) = match wrapped_response {
            ...
            Err(status) => (
                Some(HaneulError::from(status.clone())),
                Weight::zero(),
                Err(status.clone()),
            ),
        };
...
fn normalize(err: HaneulError) -> Weight {
    match err.as_inner() {
        HaneulErrorKind::UserInputError {
        ...
        | HaneulErrorKind::WrongEpoch { .. } => Weight::one(),
        _ => Weight::zero(),
    }
}
