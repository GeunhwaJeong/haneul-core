...
impl<C: Clone> SafeClient<C> {
    ...
    ) -> HaneulResult<VerifiedObjectInfoResponse> {
    ...
    C: AuthorityAPI + Send + Sync + Clone + 'static,
{
    ...
    fn verify_objects<I>(&self, objects: &Option<Vec<Object>>, expected_refs: I) -> HaneulResult
    where
    ...
    ) -> Result<PlainTransactionInfoResponse, HaneulError> {
        self.metrics
            .total_requests_handle_transaction_info_request
            .inc();
    ...
    fn verify_checkpoint_sequence(
        ...
        if let (Some(e), Some(o)) = (expected_seq, observed_seq) {
            fp_ensure!(
                e == o,
                HaneulError::from("Expected checkpoint number doesn't match with returned")
            );
        }
        Ok(())
    }
    ...
    fn verify_contents_exist<T, O>(
        ...
        match (request_content, checkpoint, contents) {
            ...
            (true, Some(_), None) | (false, _, Some(_)) | (_, None, Some(_)) => Err(
                HaneulError::from("Checkpoint contents inconsistent with request"),
            ),
            _ => Ok(()),
        }
    }
