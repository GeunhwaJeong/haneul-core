...
impl Scenario {
    ...
    fn new_outputs() -> TransactionOutputs {
        ...
        let (sender, keypair): (HaneulAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
        let (receiver, _): (HaneulAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
    ...
    pub async fn commit(&mut self, tx: TransactionDigest) -> HaneulResult {
