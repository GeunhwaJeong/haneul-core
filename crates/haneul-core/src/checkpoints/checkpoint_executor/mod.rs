    ...
    pub fn new(
        ...
        for (idx, efx) in effects.iter().enumerate() {
            ...
            let acc_version = efx.object_changes().into_iter().find_map(|change| {
                if change.id == HANEUL_ACCUMULATOR_ROOT_OBJECT_ID {
...
impl CheckpointExecutor {
    ...
    pub async fn run_epoch(self, run_with_range: Option<RunWithRange>) -> StopReason {
        let _metrics_scope = haneullabs_metrics::monitored_scope("CheckpointExecutor::run_epoch");
        ...
        let concurrency = std::env::var("HANEUL_CHECKPOINT_EXECUTION_MAX_CONCURRENCY")
            .ok()
...
impl CheckpointExecutor {
    ...
    async fn execute_checkpoint(
        ...
        if checkpoint.is_last_checkpoint_of_epoch() && sequence_number > 0 {
            let _wait_for_previous_checkpoints_guard = haneullabs_metrics::monitored_scope(
        ...
        let _parallel_step_guard =
            haneullabs_metrics::monitored_scope("CheckpointExecutor::parallel_step");
    ...
    async fn verify_locally_built_checkpoint(
        ...
        let state_hasher = {
            let _metrics_scope =
                haneullabs_metrics::monitored_scope("CheckpointExecutor::notify_read_state_hasher");
            self.epoch_store
    ...
    async fn execute_transactions_from_synced_checkpoint(
        ...
        let (mut ckpt_state, tx_data, unexecuted_tx_digests) = {
            let _scope =
                haneullabs_metrics::monitored_scope("CheckpointExecutor::execute_transactions");
        ...
        let _scope = haneullabs_metrics::monitored_scope("CheckpointExecutor::finalize_checkpoint");
