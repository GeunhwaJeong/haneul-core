...
pub struct HaneulInitArgs {
    ...
    /// URL for the Haneul REST API. To be passed to the offchain indexer and reader.
    #[clap(long)]
...
pub struct ProgrammableTransactionCommand {
    ...
    #[clap(
        long = "inputs",
        value_parser = ParsedValue::<HaneulExtraValueArgs>::parse,
        num_args(1..),
...
pub struct SetAddressCommand {
    pub address: String,
    #[clap(value_parser = ParsedValue::<HaneulExtraValueArgs>::parse)]
    ...
    fn from_arg_matches(matches: &clap::ArgMatches) -> Result<Self, clap::Error> {
        Ok(match matches.subcommand() {
            ...
            Some(("authenticator-state-update", matches)) => {
                HaneulSubcommand::AuthenticatorStateUpdate(
            ...
            Some(("run-jsonrpc", matches)) => {
                HaneulSubcommand::RunJsonRpc(RunJsonRpcCommand::from_arg_matches(matches)?)
            }
...
impl HaneulExtraValueArgs {
    ...
    fn parse_owned_object_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(HaneulExtraValueArgs::Owned(fake_id, version))
    }
    ...
    fn parse_read_shared_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(HaneulExtraValueArgs::Shared(
    ...
    fn parse_withdraw_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        // Need to track nesting level for types like Balance<Coin<HANEUL>>
...
impl HaneulValue {
    ...
    fn resolve_object(
        ...
        let obj_res = if let Some(v) = version {
            haneul_types::storage::ObjectStore::get_object_by_key(&*test_adapter.executor, &id, v)
        } else {
            haneul_types::storage::ObjectStore::get_object(&*test_adapter.executor, &id)
        };
    ...
    pub(crate) fn into_call_arg(self, test_adapter: &HaneulTestAdapter) -> anyhow::Result<CallArg> {
        Ok(match self {
            ...
            HaneulValue::Owned(fake_id, version) => {
                CallArg::Object(Self::owned_arg(fake_id, version, test_adapter)?)
            }
            HaneulValue::Shared(mutability, fake_id, version) => CallArg::Object(Self::shared_arg(
                fake_id,
                version,
                test_adapter,
                mutability,
...
impl ParsableValue for HaneulExtraValueArgs {
    ...
    fn concrete_vector(elems: Vec<Self::ConcreteValue>) -> anyhow::Result<Self::ConcreteValue> {
        if !elems.is_empty() && matches!(elems[0], HaneulValue::Object(_, _)) {
    ...
    fn concrete_struct(values: Vec<Self::ConcreteValue>) -> anyhow::Result<Self::ConcreteValue> {
        Ok(HaneulValue::MoveValue(MoveValue::Struct(MoveStruct(
    ...
    fn into_concrete_value(
        ...
        match self {
            ...
            HaneulExtraValueArgs::Owned(id, version) => Ok(HaneulValue::Owned(id, version)),
            HaneulExtraValueArgs::Shared(mutability, id, version) => {
                Ok(HaneulValue::Shared(mutability, id, version))
            }
