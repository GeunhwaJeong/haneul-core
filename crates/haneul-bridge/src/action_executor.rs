...
pub struct BridgeActionExecutor<C> {
    ...
    haneul_token_type_tags: Arc<ArcSwap<HashMap<u8, TypeTag>>>,
    ...
    C: HaneulClientInner + 'static,
{
    ...
    fn run_inner(
        ...
        Vec<tokio::task::JoinHandle<()>>,
        haneullabs_metrics::metered_channel::Sender<BridgeActionExecutionWrapper>,
        haneullabs_metrics::metered_channel::Sender<CertifiedBridgeActionExecutionWrapper>,
    ) {
        let key = self.key;
        ...
        let (execution_tx, execution_rx) = haneullabs_metrics::metered_channel::channel(
            CHANNEL_SIZE,
            &haneullabs_metrics::get_metrics()
                .unwrap()
                .channel_inflight
        ...
        let client_clone = self.haneul_client.clone();
        let mut tasks = vec![];
    ...
    async fn handle_signing_task(
        ...
        let should_proceed = Self::should_proceed_signing(haneul_client).await;
        if !should_proceed {
    ...
    async fn handle_already_processed_token_transfer_action_maybe(
        ...
        match status {
            ...
            // Although theoretically a legit HaneulToEthBridgeAction should not have
            // status `NotFound`
    ...
    // Before calling this function, `key` and `haneul_address` need to be
    // verified to match.
    async fn run_onchain_execution_loop(
        ...
        execution_queue_sender: haneullabs_metrics::metered_channel::Sender<
            CertifiedBridgeActionExecutionWrapper,
        >,
        mut execution_queue_receiver: haneullabs_metrics::metered_channel::Receiver<
            CertifiedBridgeActionExecutionWrapper,
        >,
        bridge_object_arg: ObjectArg,
        haneul_token_type_tags: Arc<ArcSwap<HashMap<u8, TypeTag>>>,
    ...
    async fn handle_execution_task(
        ...
        info!("Building Haneul transaction");
        let rgp = haneul_client.get_reference_gas_price_until_success().await;
        let tx_data = match build_haneul_transaction(
            *haneul_address,
            &gas_object_ref,
            ...
            *bridge_object_arg,
            ...
            Err(BridgeError::UnknownTokenId(token_id)) => {
                ...
                match haneul_client.get_token_id_map().await {
                    Ok(new_token_map) => {
                        haneul_token_type_tags.store(Arc::new(new_token_map));
                        // Retry building transaction with refreshed token map
                        match build_haneul_transaction(
                            *haneul_address,
                            &gas_object_ref,
                            ceriticate_clone,
                            *bridge_object_arg,
                            ...
                            Ok(tx_data) => tx_data,
                            Err(err) => {
                                metrics.err_build_haneul_transaction.inc();
                                error!(
        ...
        info!(?tx_digest, ?gas_object_ref, "Sending transaction to Haneul");
        match haneul_client
            ...
            .await
        {
            ...
            Err(err) => {
                error!(
                    ?action_key,
                    ?tx_digest,
                    "Haneul transaction failed at signing: {err:?}"
                );
                metrics.err_haneul_transaction_submission.inc();
                let metrics_clone = metrics.clone();
                ...
                spawn_logged_monitored_task!(async move {
                    ...
                    if attempt_times >= MAX_EXECUTION_ATTEMPTS {
                        metrics_clone
                            .err_haneul_transaction_submission_too_many_failures
                            .inc();
    ...
    async fn handle_execution_effects(
        ...
        match &response.status {
            ...
            HaneulExecutionStatus::Failure { error } => {
                ...
                metrics.err_haneul_transaction_execution.inc();
                error!(
                    ?tx_digest,
                    "Manual intervention is needed. Haneul transaction executed and failed with error: {error:?}"
                );
            }
        }
    }
    ...
    async fn get_gas_data_assert_ownership(
        haneul_address: HaneulAddress,
        gas_object_id: ObjectID,
        haneul_client: &HaneulClient<C>,
    ) -> (GasCoin, ObjectRef) {
        let (gas_coin, gas_obj_ref, owner) = haneul_client
...
pub async fn submit_to_executor(
    tx: &haneullabs_metrics::metered_channel::Sender<BridgeActionExecutionWrapper>,
    action: BridgeAction,
) -> Result<(), BridgeError> {
...
mod tests {
    ...
    async fn test_onchain_execution_loop() {
        ...
        let id_token_map = (*haneul_token_type_tags.load().clone()).clone();
        ...
        let tx_digest = get_tx_digest(tx_data, &dummy_haneul_key);

        let gas_coin = GasCoin::new_for_testing(1_000_000_000_000); // dummy gas coin
        haneul_client_mock.add_gas_object_info(
            gas_coin.clone(),
            gas_object_ref,
            Owner::AddressOwner(haneul_address),
        );
    ...
    async fn test_signature_aggregation_loop() {
        ...
        loop {
            let requested_times =
                mock0.get_haneul_token_events_requested(haneul_tx_digest, haneul_tx_event_index);
    ...
    async fn test_skip_request_signature_if_already_processed_on_chain() {
        ...
        haneul_client_mock.set_action_onchain_status(&action, BridgeActionStatus::Approved);
    ...
    async fn test_skip_tx_submission_if_already_processed_on_chain() {
        ...
        haneul_client_mock.set_action_onchain_status(&action, BridgeActionStatus::Pending);
    ...
    async fn test_skip_tx_submission_if_bridge_is_paused() {
        ...
        let id_token_map: HashMap<u8, TypeTag> = (*haneul_token_type_tags.load().clone()).clone();
        let (action_certificate, _, _) = get_bridge_authority_approved_action(
            vec![&mock0, &mock1, &mock2, &mock3],
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
            None,
    ...
    async fn test_action_executor_handle_new_token() {
        ...
        let (action_certificate, _, _) = get_bridge_authority_approved_action(
            ...
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
            Some(new_token_id),
            false, // we need an eth -> haneul action that entails the new token type tag in transaction building
        );
        ...
        haneul_token_type_tags.store(Arc::new(id_token_map));
    ...
    fn mock_bridge_authority_sigs(
        ...
        for (mock, secret) in mocks.iter().zip(secrets.iter()) {
            ...
            mock.add_haneul_event_response(
                haneul_tx_digest,
                haneul_tx_event_index,
    ...
    fn get_bridge_authority_approved_action(
        ...
        let haneul_tx_digest = TransactionDigest::random();
        let haneul_tx_event_index = 1;
        let action = if haneul_to_eth {
    ...
    fn mock_transaction_response(
        haneul_client_mock: &HaneulMockClient,
        tx_digest: TransactionDigest,
        ...
        if wildcard {
            ...
            haneul_client_mock.add_transaction_response(tx_digest, Ok(response));
        }
    }
    ...
    fn mock_transaction_error(
        ...
        if wildcard {
            haneul_client_mock.set_wildcard_transaction_response(Err(error));
        } else {
            haneul_client_mock.add_transaction_response(tx_digest, Err(error));
        }
    }
    ...
    async fn setup() -> (
        ...
        let (haneul_address, kp): (_, fastcrypto::secp256k1::Secp256k1KeyPair) = get_key_pair();
        let haneul_key = HaneulKeyPair::from(kp);
        let gas_object_ref = random_object_ref();
        let temp_dir = tempfile::tempdir().unwrap();
        let store = BridgeOrchestratorTables::new(temp_dir.path());
        let haneul_client_mock = HaneulMockClient::default();
        let tx_subscription = haneul_client_mock.subscribe_to_requested_transactions();
        let haneul_client = Arc::new(HaneulClient::new_for_testing(haneul_client_mock.clone()));
        ...
        let dummy_haneul_key = HaneulKeyPair::from(dummy_kp);
