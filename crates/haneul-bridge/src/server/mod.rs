...
pub const HANEUL_TO_ETH_TRANSFER_PATH: &str =
...
pub const ADD_TOKENS_ON_HANEUL_PATH: &str = "/sign/add_tokens_on_haneul/{chain_id}/{nonce}/{native}/{token_ids}/{token_type_names}/{token_prices}";
pub const ADD_TOKENS_ON_EVM_PATH: &str = "/sign/add_tokens_on_evm/{chain_id}/{nonce}/{native}/{token_ids}/{token_addresses}/{token_haneul_decimals}/{token_prices}";
...
pub(crate) fn make_router(
    ...
    Router::new()
        ...
        .route(ADD_TOKENS_ON_HANEUL_PATH, get(handle_add_tokens_on_haneul))
    ...
    fn into_response(self) -> axum::response::Response {
        ...
        let sanitized_error = match self {
            ...
            BridgeError::BridgeEventInUnrecognizedHaneulPackage => {
                "BridgeEventInUnrecognizedHaneulPackage"
            }
...
async fn handle_haneul_tx_digest(
    ...
    let future = async {
        let sig: Json<SignedBridgeAction> = handler
            .handle_haneul_tx_digest(tx_digest_base58, event_idx)
            .await?;
        Ok(sig)
    };
    with_metrics!(metrics.clone(), "handle_haneul_tx_digest", future).await
}
...
async fn handle_haneul_token_transfer(
    ...
    let future = async {
        let sig: Json<SignedBridgeAction> = handler
            .handle_haneul_token_transfer(source_chain, message_type, bridge_seq_num)
            .await?;
        Ok(sig)
    };
    with_metrics!(metrics.clone(), "handle_haneul_token_transfer", future).await
}
...
async fn handle_add_tokens_on_haneul(
    ...
    let future = async {
        ...
        if !chain_id.is_haneul_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_haneul only expects Haneul chain id".to_string(),
            ));
        }
    ...
    with_metrics!(metrics.clone(), "handle_add_tokens_on_haneul", future).await
}
...
async fn handle_add_tokens_on_evm(
    ...
    let future = async {
        ...
        if chain_id.is_haneul_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_evm does not expect Haneul chain id".to_string(),
            ));
        }
        ...
        validate_list_size(&token_haneul_decimals, "token_haneul_decimals")?;
        validate_list_size(&token_prices, "token_prices")?;
        ...
        let token_addresses = token_addresses
            ...
            .collect::<Result<Vec<_>, _>>()?;
        let token_haneul_decimals = token_haneul_decimals
            .split(',')
            .map(|s| {
                s.parse::<u8>().map_err(|err| {
                    BridgeError::InvalidBridgeClientRequest(format!(
                        "Invalid token haneul decimals: {:?}",
                        err
                    ))
                })
            })
...
mod tests {
    ...
    #[tokio::test]
    async fn test_bridge_server_handle_add_tokens_on_haneul_action_path() {
        let client = setup();
