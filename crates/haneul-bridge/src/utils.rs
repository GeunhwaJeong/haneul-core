...
use crate::types::{AddTokensOnHaneulAction, BridgeAction, BridgeCommittee};
...
pub struct EthBridgeContracts {
    pub bridge: EthHaneulBridge::EthHaneulBridgeInstance<EthProvider>,
...
/// If a validator is not in the Haneul committee, we will use its base URL as the name.
pub async fn get_committee_voting_power_by_name(
    ...
    let mut haneul_committee: BTreeMap<_, _> = system_state
        .active_validators
        .iter()
    ...
    bridge_committee
        ...
        .map(|(name, validator)| {
            (
                name.clone(),
                haneul_committee
                    .remove(&validator.haneul_address)
