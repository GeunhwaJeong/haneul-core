...
use crate::abi::{
    EthBridgeCommittee, EthBridgeConfigInstance, EthBridgeEvent, EthERC20, EthHaneulBridge,
    EthHaneulBridgeEvents,
};
use crate::config::{BridgeNodeConfig, EthConfig, HaneulConfig, default_ed25519_key_pair};
use crate::crypto::{
...
use crate::utils::{
    ...
    publish_and_register_coins_return_add_coins_on_haneul_action, wait_for_server_to_be_up,
};
...
const HANEUL_BRIDGE_NAME: &str = "HaneulBridge";
...
pub struct BridgeTestCluster {
    ...
    bridge_client: HaneulBridgeClient,
...
impl BridgeTestClusterBuilder {
    ...
    pub fn with_haneul_chain_id(mut self, chain_id: BridgeChainId) -> Self {
        self.haneul_chain_id = chain_id;
        self
    }
...
impl BridgeTestCluster {
    ...
    pub fn bridge_client(&self) -> &HaneulBridgeClient {
        &self.bridge_client
    }
    ...
    pub fn haneul_chain_id(&self) -> BridgeChainId {
        self.haneul_chain_id
    }
    ...
    pub fn haneul_rpc_url(&self) -> String {
    ...
    pub async fn upgrade_bridge_to_v2(&self) -> anyhow::Result<()> {
        ...
        let bridge_contract = EthHaneulBridge::new(haneul_bridge_proxy, eth_signer.clone());
...
impl DeployedSolContracts {
    ...
    pub fn haneul_bridge_addrress_hex(&self) -> String {
        Self::eth_adress_to_hex(self.haneul_bridge)
    }
}
...
pub(crate) async fn deploy_sol_contract(
    ...
    let contracts = DeployedSolContracts {
        haneul_bridge: deployed_contracts.remove(HANEUL_BRIDGE_NAME).unwrap(),
...
impl EthBridgeEnvironment {
    ...
    pub async fn get_supported_token(&self, token_id: u8) -> (EthAddress, u8, u64) {
        ...
        let token_haneul_decimal = config.tokenHaneulDecimalOf(token_id).call().await.unwrap();
        let token_price = config.tokenPriceOf(token_id).call().await.unwrap();
        (token_address, token_haneul_decimal, token_price)
    }
}
...
pub(crate) async fn start_bridge_cluster(
        ...
        .enumerate()
    {
        ...
        let config = BridgeNodeConfig {
            ...
            haneul: HaneulConfig {
                haneul_rpc_url: test_cluster.inner.fullnode_handle.rpc_url.clone(),
...
pub async fn get_signatures(
    ...
    let sigs = haneul_bridge_client
        .get_token_transfer_action_onchain_signatures_until_success(haneul_chain_id, nonce)
        .await
        .unwrap();
...
async fn initiate_bridge_eth_to_haneul_internal(
    ...
    let haneul_amount = (U256::from(amount) * U256::from(10).pow(U256::from(8))).to::<u64>(); // DP for Ether on Haneul
    ...
    let haneul_recipient_address = haneul_address.to_vec().into();
    let tx_receipt = if use_v2 {
        let call = contract
            .bridgeETHV2(haneul_recipient_address, haneul_chain_id as u8)
            .value(eth_amount);
        send_eth_tx_and_get_tx_receipt(eth_signer, call.into_transaction_request()).await
    } else {
        let call = contract
            .bridgeETH(haneul_recipient_address, haneul_chain_id as u8)
            .value(eth_amount);
        send_eth_tx_and_get_tx_receipt(eth_signer, call.into_transaction_request()).await
    };
    ...
    let (
        ...
        EthBridgeEvent::EthHaneulBridgeEvents(EthHaneulBridgeEvents::TokensDeposited(event)) => (
            event.sourceChainID,
            event.destinationChainID,
            event.tokenID,
            event.haneulAdjustedAmount,
            event.senderAddress,
            event.recipientAddress.to_vec(),
            None,
        ),
        EthBridgeEvent::EthHaneulBridgeEvents(EthHaneulBridgeEvents::TokensDepositedV2(event)) => (
            event.sourceChainID,
            event.destinationChainID,
            event.tokenID,
            event.haneulAdjustedAmount,
...
pub async fn initiate_bridge_erc20_to_haneul(
    ...
    let haneul_amount = amount.to::<u64>();
    ...
    assert_eq!(eth_bridge_event.destinationChainID, haneul_chain_id as u8);
    assert_eq!(eth_bridge_event.tokenID, token_id);
    assert_eq!(eth_bridge_event.haneulAdjustedAmount, haneul_amount);
