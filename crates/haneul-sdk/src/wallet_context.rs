...
use crate::haneul_client_config::{HaneulClientConfig, HaneulEnv};
...
impl WalletContext {
    ...
    pub fn new_for_tests(
        ...
        let mut config = HaneulClientConfig::new(keystore)
    ...
    pub fn addresses_with_alias(&self) -> Vec<(&HaneulAddress, &Alias)> {
    ...
    async fn internal_load_or_cache_chain_id(
        ...
        let mut config_result = HaneulClientConfig::load_with_lock(path)?;
    ...
    pub async fn infer_sender(&mut self, gas: &[ObjectID]) -> Result<HaneulAddress, anyhow::Error> {
        if gas.is_empty() {
    ...
    pub async fn gas_for_owner_budget(
        ...
        Err(anyhow!(
            "No non-argument gas objects found for this address with value >= budget {budget}. Run haneul client gas to check for gas objects."
        ))
    }
    ...
    pub async fn get_one_account(&self) -> anyhow::Result<(HaneulAddress, Vec<ObjectRef>)> {
    ...
    pub async fn get_one_gas_object(&self) -> anyhow::Result<Option<(HaneulAddress, ObjectRef)>> {
    ...
    pub async fn get_all_accounts_and_gas_objects(
        &self,
    ) -> anyhow::Result<Vec<(HaneulAddress, Vec<ObjectRef>)>> {
        let mut result = vec![];
        for address in self.get_addresses() {
            let objects = self
                .gas_objects(address)
    ...
    /// Add an account
    pub async fn add_account(&mut self, alias: Option<String>, keypair: HaneulKeyPair) {
