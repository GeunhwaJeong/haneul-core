...
use haneul_sdk::{
    haneul_client_config::HaneulClientConfig,
...
async fn test_get_wallet_key() {
    ...
    let signature = wallet_context
        .sign_secure(&alias_identity, &transaction, Intent::haneul_transaction())
        .await
        .unwrap();

    let intent_message = IntentMessage::new(Intent::haneul_transaction(), transaction.clone());
...
async fn test_update_env_chain_id_new_chain_id() -> Result<(), anyhow::Error> {
    ...
    let config = HaneulClientConfig::load(&config_path)?;
    ...
    let reloaded_config = HaneulClientConfig::load(&config_path)?;
...
async fn test_update_env_chain_id_overwrite_existing() -> Result<(), anyhow::Error> {
    ...
    let mut config = HaneulClientConfig::load(&config_path)?;
    ...
    let final_config = HaneulClientConfig::load(&config_path)?;
    assert_eq!(
