...
use haneul_indexer_alt_schema::objects::StoredCoinBalanceBucket;
use haneul_indexer_alt_schema::objects::StoredCoinBalanceBucketDeletionReference;
use haneul_indexer_alt_schema::objects::StoredCoinOwnerKind;
use haneul_indexer_alt_schema::schema::coin_balance_buckets;
use haneul_indexer_alt_schema::schema::coin_balance_buckets_deletion_reference;
...
impl Handler for CoinBalanceBuckets {
    ...
    async fn prune<'a>(
        ...
        //
        // TODO: use haneul_sql_macro's query!
        let query = format!(
            "
...
pub(crate) fn get_coin_owner(object: &Object) -> Option<(StoredCoinOwnerKind, HaneulAddress)> {
    match object.owner() {
...
mod tests {
    ...
    use haneul_indexer_alt_framework::types::object::MoveObject;
    use haneul_indexer_alt_framework::types::object::Object;
    ...
    fn test_get_coin_balance_bucket() {
        ...
        let zero_coin = Object::with_id_owner_gas_for_testing(id, HaneulAddress::ZERO, 0);
        assert_eq!(get_coin_balance_bucket(&zero_coin).unwrap(), 0);

        // Test coin with balance 1 (10^0)
        let one_coin = Object::with_id_owner_gas_for_testing(id, HaneulAddress::ZERO, 1);
        assert_eq!(get_coin_balance_bucket(&one_coin).unwrap(), 0);

        // Test coin with balance 100 (10^2)
        let hundred_coin = Object::with_id_owner_gas_for_testing(id, HaneulAddress::ZERO, 100);
        assert_eq!(get_coin_balance_bucket(&hundred_coin).unwrap(), 2);

        // Test coin with balance 1000000 (10^6)
        let million_coin = Object::with_id_owner_gas_for_testing(id, HaneulAddress::ZERO, 1000000);
        assert_eq!(get_coin_balance_bucket(&million_coin).unwrap(), 6);

        // The type of this object is a staked HANEUL, not a coin.
        let invalid_coin = unsafe {
            Object::new_move(
                MoveObject::new_from_execution(
                    MoveObjectType::staked_haneul(),
                    false,
                    SequenceNumber::new(),
    ...
    fn test_get_coin_owner() {
        let id = ObjectID::random();
        let addr1 = HaneulAddress::random_for_testing_only();
    ...
    #[tokio::test]
    async fn test_process_coin_balance_buckets_new_haneul_coin() {
        let (indexer, _db) = Indexer::new_for_testing(&MIGRATIONS).await;
        let mut conn = indexer.store().connect().await.unwrap();
        let mut builder = TestCheckpointBuilder::new(0);
        builder = builder
            .start_transaction(0)
            .create_haneul_object(0, 0)
            .create_haneul_object(1, 100)
            .finish_transaction();
    ...
    async fn test_process_coin_balance_buckets_balance_change() {
        ...
        // Transfer 10 GEUNHWA, balance goes from 10010 to 10000.
        ...
        // Transfer 1 GEUNHWA, balance goes from 10000 to 9999.
    ...
    async fn test_process_coin_balance_buckets_out_of_order_pruning() {
        ...
        builder = builder
            .start_transaction(0)
            .create_haneul_object(0, 100)
            .create_haneul_object(1, 1000)
            .create_haneul_object(2, 10000)
            .finish_transaction();
