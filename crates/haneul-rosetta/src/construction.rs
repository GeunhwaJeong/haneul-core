...
pub async fn derive(
    ...
    let address: HaneulAddress = request.public_key.try_into()?;
...
pub async fn combine(
    ...
    let signed_tx = haneul_types::transaction::Transaction::from_generic_sig_data(
        intent_msg.value,
...
pub async fn submit(
    ...
    env.check_network_identifier(&request.network_identifier)?;
    let signed_tx: haneul_types::transaction::Transaction =
...
pub async fn hash(
    ...
    let tx: haneul_types::transaction::Transaction = bcs::from_bytes(&tx_bytes)?;
...
/// For Haneul, we are returning the latest object refs for all the input objects,
...
pub async fn metadata(
    ...
    let is_pay_haneul_or_stake = matches!(
        &option.internal_operation,
        InternalOperation::PayHaneul(_) | InternalOperation::Stake(_)
    );
    ...
    // For PayHaneul/Stake: extra_gas_coins contains the coins to merge (same as objects)
    // For PayCoin/WithdrawStake: extra_gas_coins is empty
    let extra_gas_coins = if is_pay_haneul_or_stake {
        objects.clone()
    } else {
        vec![]
    };
    ...
    Ok(ConstructionMetadataResponse {
        metadata: ConstructionMetadata {
            ...
            objects,
            party_objects,
            total_coin_value: total_haneul_balance,
            gas_price,
            budget,
            currency,
        },
...
pub async fn parse(
    ...
    let (data, sender) = if request.signed {
        let tx: haneul_types::transaction::Transaction =
