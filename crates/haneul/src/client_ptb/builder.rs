...
impl<'a> PTBBuilder<'a> {
    ...
    fn declare_possible_address_binding(&mut self, ident: String, possible_addr: &Spanned<PTBArg>) {
        match possible_addr.value {
            ...
            // If we encounter a dotted string e.g., "foo.0" or "haneul.io" or something like that
...
pub fn is_mvr_name(name: &str) -> bool {
    name.contains('/') && (name.contains("@") || name.contains(".haneul"))
}
...
/// ) or the known package names (e.g., haneul, deepbook, etc) by calling `resolve_address`.
