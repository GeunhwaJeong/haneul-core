...
use crate::{
    client_commands::{
        GasDataArgs, HaneulClientCommandResult, TxProcessingArgs, dry_run_or_execute_or_serialize,
    },
    client_ptb::{
...
impl PTB {
    ...
    pub async fn execute(self, context: &mut WalletContext) -> Result<(), Error> {
        ...
        let transaction_response = match transaction_response {
            HaneulClientCommandResult::ComputeTransactionDigest(_)
            | HaneulClientCommandResult::DryRun(_)
        ...
        let command_result = HaneulClientCommandResult::TransactionBlock(transaction_response);
...
pub fn ptb_description() -> clap::Command {
    clap::Command::new("haneul client ptb")
        .about(
        ...
        .arg(arg!(
            --"gas-budget" <GEUNHWA>
            "An optional gas budget for this PTB (in GEUNHWA). If gas budget is not provided, the \
        ...
        .arg(arg!(
            --"gas-price" <GEUNHWA>
            "An optional gas price for this PTB (in GEUNHWA). If not specified, the reference gas price \
            is fetched from RPC."
        ))
        .arg(arg!(
            --"gas-sponsor" <ADDRESS>
        ...
        .long_help(
            ...
            \n --make-move-vec <haneul::coin::Coin<haneul::haneul::HANEUL>> [gas]"
        )
