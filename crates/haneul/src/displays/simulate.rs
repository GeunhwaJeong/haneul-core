...
use haneul_types::{effects::TransactionEffectsAPI, transaction::TransactionDataAPI};
...
impl Display for Pretty<'_, SimulateTransactionResponse> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        match response.transaction.effects.status() {
            haneul_types::execution_status::ExecutionStatus::Success => writeln!(f, "success")?,
            haneul_types::execution_status::ExecutionStatus::Failure { error, command } => {
                writeln!(f, "failure")?;
