...
use alloy::primitives::{Address as EthAddress, B256};
use alloy::providers::Provider;
use alloy::rpc::types::Log;
use alloy::rpc::types::{Block, Filter, Transaction};
use anyhow::Error;
...
use haneul_bridge::error::BridgeError;
use haneul_bridge::eth_client::EthClient;
use haneul_bridge::eth_syncer::EthSyncer;
...
impl EthSubscriptionDatasource {
    pub async fn new(
        eth_haneul_bridge_contract_addresses: Vec<EthAddress>,
        eth_client: Arc<EthClient>,
        eth_ws_url: String,
        metrics: Box<dyn IndexerMetricProvider>,
        genesis_block: u64,
        ...
        Ok(Self {
            addresses: eth_haneul_bridge_contract_addresses,
            eth_client,
            eth_ws_url,
            metrics,
            genesis_block,
...
impl EthFinalizedSyncDatasource {
    pub async fn new(
        ...
        Ok(Self {
            bridge_addresses: eth_haneul_bridge_contract_addresses,
            eth_http_url,
            eth_client,
            metrics,
            bridge_metrics,
    ...
    fn map(
        ...
        match bridge_event {
            EthBridgeEvent::EthHaneulBridgeEvents(bridge_event) => match &bridge_event {
                EthHaneulBridgeEvents::TokensDeposited(bridge_event) => {
                    info!(
                    ...
                    processed_txn_data.push(ProcessedTxnData::TokenTransfer(TokenTransfer {
                        ...
                        data: Some(TokenTransferData {
                            ...
                            amount: bridge_event.haneulAdjustedAmount,
                            is_finalized,
                        }),
                    }));
                }
                EthHaneulBridgeEvents::TokensDepositedV2(bridge_event) => {
                    info!(
                        "Observed Eth Deposit at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                ...
                EthHaneulBridgeEvents::TokensClaimed(bridge_event) => {
                    info!(
                ...
                EthHaneulBridgeEvents::EmergencyOperation(f) => {
                    info!(
                ...
                EthHaneulBridgeEvents::ContractUpgraded(f) => {
                    info!(
                        "Observed Eth HaneulBridge Upgrade at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
                ...
                EthHaneulBridgeEvents::Initialized(_)
                | EthHaneulBridgeEvents::Paused(_)
                | EthHaneulBridgeEvents::Unpaused(_)
                | EthHaneulBridgeEvents::Upgraded(_) => {
            ...
            EthBridgeEvent::EthBridgeConfigEvents(bridge_event) => match &bridge_event {
                ...
                EthBridgeConfigEvents::TokenAdded(_) => {
                    info!(
                        "Observed Eth AddHaneulTokens at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
