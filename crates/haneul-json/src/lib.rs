...
pub enum HaneulJsonValueErrorKind {
...
#[derive(Debug)]
pub struct HaneulJsonValueError {
    kind: HaneulJsonValueErrorKind,
    val: JsonValue,
}

impl HaneulJsonValueError {
    pub fn new(val: &JsonValue, kind: HaneulJsonValueErrorKind) -> Self {
        Self {
            kind,
            val: val.clone(),
        }
...
impl std::error::Error for HaneulJsonValueError {}

impl fmt::Display for HaneulJsonValueError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err_str = match self.kind {
            HaneulJsonValueErrorKind::ValueTypeNotAllowed => {
                format!("JSON value type {} not allowed.", self.val)
            }
            HaneulJsonValueErrorKind::ArrayNotHomogeneous => {
...
impl HaneulJsonValue {
    ...
    pub fn from_object_id(id: ObjectID) -> HaneulJsonValue {
    ...
    pub fn to_move_value(
        ...
        Ok(match (val, ty) {
                ...
                if struct_layout.type_ == ID::type_() =>
            {
                ...
                let addr = HaneulAddress::from_str(s)?;
    ...
    match value {
        ...
        JsonValue::Array(bytes) => {
            ...
            let bytes = value_to_byte_array(bytes)
                .ok_or_else(|| anyhow!("Invalid input: Cannot parse input into HaneulAddress."))?;
            Ok(HaneulAddress::try_from(bytes)?)
        }
...
fn move_value_to_json(move_value: &MoveValue) -> Option<JsonValue> {
    Some(match move_value {
        ...
        MoveValue::Signer(v) | MoveValue::Address(v) => json!(HaneulAddress::from(*v).to_string()),
        ...
        MoveValue::Struct(move_struct) => match move_struct {
            ...
            MoveStruct { fields, type_ } if type_ == &ID::type_() => {
                ...
                if let MoveValue::Address(address) = v {
                    json!(HaneulAddress::from(*address))
                } else {
                    return None;
                }
            }
...
impl FromStr for HaneulJsonValue {
    ...
    fn from_str(s: &str) -> Result<Self, anyhow::Error> {
        ...
        HaneulJsonValue::new(serde_json::from_str(s).unwrap_or_else(|_| try_escape_array(s)))
    }
}
...
pub fn check_valid_homogeneous(val: &JsonValue) -> Result<(), HaneulJsonValueError> {
...
fn check_valid_homogeneous_rec(
    ...
    while let Some(v) = curr_q.pop_front() {
        let curr = match v {
            ...
            // Not valid
            _ => {
                return Err(HaneulJsonValueError::new(
                    v,
                    HaneulJsonValueErrorKind::ValueTypeNotAllowed,
                ));
            }
...
pub fn primitive_type(
    ...
    Some(match param {
        ...
        SignatureToken::Datatype(struct_handle_idx) => {
            ...
            } else if resolved_struct == RESOLVED_HANEUL_ID {
...
macro_rules! call_args {
        ($($value:expr),*) => {
        Ok::<_, anyhow::Error>(vec![$(haneul_json::call_arg!($value)?,)*])
    };
    }
...
macro_rules! call_arg {
    ($value:expr) => {{
        ...
        trait HaneulJsonArg {
            fn to_haneul_json(&self) -> anyhow::Result<HaneulJsonValue>;
        }
        // TODO: anyway to condense this?
        impl HaneulJsonArg for &str {
            fn to_haneul_json(&self) -> anyhow::Result<HaneulJsonValue> {
                HaneulJsonValue::from_str(self)
            }
        }
        impl HaneulJsonArg for String {
            fn to_haneul_json(&self) -> anyhow::Result<HaneulJsonValue> {
                HaneulJsonValue::from_str(&self)
            }
        }
        impl HaneulJsonArg for haneul_types::base_types::ObjectID {
            fn to_haneul_json(&self) -> anyhow::Result<HaneulJsonValue> {
                HaneulJsonValue::from_str(&self.to_string())
            }
        }
        impl HaneulJsonArg for haneul_types::base_types::HaneulAddress {
            fn to_haneul_json(&self) -> anyhow::Result<HaneulJsonValue> {
                HaneulJsonValue::from_str(&self.to_string())
            }
        }
        impl HaneulJsonArg for u64 {
            fn to_haneul_json(&self) -> anyhow::Result<HaneulJsonValue> {
                HaneulJsonValue::from_bcs_bytes(
                    Some(&haneul_json::MoveTypeLayout::U64),
                    &bcs::to_bytes(self)?,
                )
            }
        }
        impl HaneulJsonArg for Vec<u8> {
            fn to_haneul_json(&self) -> anyhow::Result<HaneulJsonValue> {
                HaneulJsonValue::from_bcs_bytes(None, &self)
            }
        }
        impl HaneulJsonArg for &[u8] {
            fn to_haneul_json(&self) -> anyhow::Result<HaneulJsonValue> {
                HaneulJsonValue::from_bcs_bytes(None, self)
            }
        }
        $value.to_haneul_json()
    }};
}
...
macro_rules! type_args {
    ($($value:expr), *) => {{
        ...
        trait HaneulJsonTypeArg {
            fn to_haneul_json(&self) -> anyhow::Result<HaneulTypeTag>;
        }
        impl <T: core::fmt::Display> HaneulJsonTypeArg for T {
            fn to_haneul_json(&self) -> anyhow::Result<HaneulTypeTag> {
                Ok(haneul_types::parse_haneul_type_tag(&self.to_string())?.into())
            }
        }
        Ok::<_, anyhow::Error>(vec![$($value.to_haneul_json()?,)*])
    }};
    }
