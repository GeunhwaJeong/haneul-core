...
impl Display for HaneulTransactionBlockResponse {
...
pub enum HaneulTransactionBlockKind {
    ...
    ConsensusCommitPrologueV3(HaneulConsensusCommitPrologueV3),
    ConsensusCommitPrologueV4(HaneulConsensusCommitPrologueV4),

    ProgrammableSystemTransaction(HaneulProgrammableTransactionBlock),
    ...
    fn try_from_inner(tx: TransactionKind) -> Result<Self, anyhow::Error> {
        Ok(match tx {
            ...
            TransactionKind::ConsensusCommitPrologueV3(p) => {
                Self::ConsensusCommitPrologueV3(HaneulConsensusCommitPrologueV3 {
                    epoch: p.epoch,
                    round: p.round,
            ...
            TransactionKind::ConsensusCommitPrologueV4(p) => {
                Self::ConsensusCommitPrologueV4(HaneulConsensusCommitPrologueV4 {
                    epoch: p.epoch,
                    round: p.round,
                    sub_dag_index: p.sub_dag_index,
                    commit_timestamp_ms: p.commit_timestamp_ms,
...
fn objref_string(obj: &HaneulObjectRef) -> String {
    format!(
        " ┌──\n │ ID: {} \n │ Version: {} \n │ Digest: {}\n └──",
        obj.object_id,
...
impl Display for HaneulTransactionBlockEvents {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
...
impl Display for HaneulGasData {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        writeln!(f, "Gas Owner: {}", self.owner)?;
        writeln!(f, "Gas Budget: {} GEUNHWA", self.budget)?;
        writeln!(f, "Gas Price: {} GEUNHWA", self.price)?;
...
impl HaneulTransactionBlockData {
    ...
    pub fn try_from_with_module_cache(
        ...
        let transaction = HaneulTransactionBlockKind::try_from_with_module_cache(
...
impl HaneulTransactionBlock {
    pub fn try_from(
        ...
        Ok(Self {
            data: HaneulTransactionBlockData::try_from_with_module_cache(
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct HaneulConsensusCommitPrologueV3 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct HaneulConsensusCommitPrologueV4 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename = "HaneulTransaction")]
pub enum HaneulCommand {
    /// A call to either an entry or a public Move function
    MoveCall(Box<HaneulProgrammableMoveCall>),
    ...
    Upgrade(Vec<ObjectID>, ObjectID, HaneulArgument),
    /// `forall T: Vec<T> -> vector<T>`
    /// Given n-values of the same type, it constructs a vector. For non objects or an empty vector,
    /// the type tag must be specified.
    MakeMoveVec(Option<String>, Vec<HaneulArgument>),
}

impl Display for HaneulCommand {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MoveCall(p) => {
                write!(f, "MoveCall({p})")
...
impl From<Command> for HaneulCommand {
    fn from(value: Command) -> Self {
        match value {
            Command::MoveCall(m) => HaneulCommand::MoveCall(Box::new((*m).into())),
            Command::TransferObjects(args, arg) => HaneulCommand::TransferObjects(
            ...
            Command::Publish(_modules, dep_ids) => HaneulCommand::Publish(dep_ids),
            Command::MakeMoveVec(tag_opt, args) => HaneulCommand::MakeMoveVec(
                tag_opt.map(|tag| tag.to_string()),
                args.into_iter().map(HaneulArgument::from).collect(),
            ),
            Command::Upgrade(_modules, dep_ids, current_package_id, ticket) => {
                HaneulCommand::Upgrade(dep_ids, current_package_id, HaneulArgument::from(ticket))
            }
        }
    }
}
...
impl Display for HaneulArgument {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
...
impl From<Argument> for HaneulArgument {
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct HaneulProgrammableMoveCall {
    ...
    pub arguments: Vec<HaneulArgument>,
}
...
impl Display for HaneulProgrammableMoveCall {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let Self {
            package,
            module,
...
impl From<ProgrammableMoveCall> for HaneulProgrammableMoveCall {
    fn from(value: ProgrammableMoveCall) -> Self {
        ...
        Self {
            ...
            arguments: arguments.into_iter().map(HaneulArgument::from).collect(),
        }
    }
}
...
pub enum HaneulCallArg {
    ...
    FundsWithdrawal(HaneulFundsWithdrawalArg),
}
...
impl HaneulPureValue {
    pub fn value(&self) -> HaneulJsonValue {
        self.value.clone()
    }
...
#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub enum HaneulWithdrawalTypeArg {
...
#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct HaneulFundsWithdrawalArg {
    pub reservation: HaneulReservation,
    pub type_arg: HaneulWithdrawalTypeArg,
