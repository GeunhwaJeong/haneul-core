...
fn main() {
    ...
    let runtimes = HaneulRuntimes::new(&config);
    ...
    info!("Haneul Node version: {VERSION}");
    info!(
    ...
    // if it deadlocks.
    let node_once_cell = Arc::new(AsyncOnceCell::<Arc<haneul_node::HaneulNode>>::new());
    ...
    runtimes.haneul_node.spawn(async move {
        match haneul_node::HaneulNode::start_async(config, registry_service, server_version).await {
        ...
        // when we get a shutdown signal from haneul-node, forward it on to the runtime_shutdown_channel here in
        // main to signal runtimes to all shutdown.
        tokio::select! {
           _ = shutdown_rx.recv() => {
                runtime_shutdown_tx.send(()).expect("failed to forward shutdown signal from haneul-node to haneul-node main");
            }
        }
