...
pub(crate) async fn replay_transaction<S: ReadDataStore>(
    ...
    if let haneul_types::transaction::TransactionKind::ProgrammableTransaction(ptb) =
...
// version to Object.
// Returns also the packages of the type parameters instantiated (e.g. `HANEUL` in `Coin<HANEUL>`).
