...
//! provides the [`Simulacrum`] type which is a implementation or instantiation of a haneul
...
/// A `Simulacrum` of Haneul.
///
/// This type represents a simulated instantiation of a Haneul blockchain that needs to be driven
...
impl<R, S: store::SimulatorStore> Simulacrum<R, S> {
    pub fn new_with_network_config_store(config: &NetworkConfig, rng: R, store: S) -> Self {
        ...
        let epoch_state = EpochState::new(genesis.haneul_system_object());
    ...
    /// # fn main() {
    /// let mut simulacrum = Simulacrum::new();
    /// let address = HaneulAddress::generate(simulacrum.rng());
    /// simulacrum.request_gas(address, GEUNHWA_PER_HANEUL).unwrap();
    ///
    /// // `account` now has a Coin<HANEUL> object with single HANEUL in it.
    /// // ...
    /// # }
    /// ```
    pub fn request_gas(
        ...
        let object = self
            ...
            .ok_or_else(|| {
                anyhow!("unable to find a coin with enough to satisfy request for {amount} Geunhwa")
            })?;
        ...
        let kind = haneul_types::transaction::TransactionKind::ProgrammableTransaction(pt);
        let tx_data =
            haneul_types::transaction::TransactionData::new_with_gas_data(kind, *sender, gas_data);
...
impl Simulacrum {
    ...
    /// existing code for generating transactions in haneul-test-transaction-builder by defining a trait
    /// that both WalletContext and Simulacrum implement. Then we can remove this function.
    pub fn transfer_txn(&mut self, recipient: HaneulAddress) -> (Transaction, u64) {
        let (sender, key) = self.keystore().accounts().next().unwrap();
        let sender = *sender;
