...
use haneul_json_rpc_types::{
    EventPage, HaneulEvent, HaneulTransactionBlockEffectsAPI, HaneulTransactionBlockResponse,
...
async fn test_full_node_follows_txes() -> Result<(), anyhow::Error> {
    ...
    // TODO: test fails on CI due to flakiness without this. Once https://github.com/GeunhwaJeong/haneul/pull/7056 is
...
async fn test_full_node_event_query_by_module_ok() {
    ...
    let page: EventPage = jsonrpc_client
        .request("haneulx_queryEvents", params)
        .await
        .unwrap();
...
async fn test_execute_tx_with_serialized_signature() -> Result<(), anyhow::Error> {
    ...
    for txn in txns {
        ...
        let response: HaneulTransactionBlockResponse = jsonrpc_client
            .request("haneul_executeTransactionBlock", params)
            .await
            .unwrap();
...
async fn test_full_node_transaction_orchestrator_rpc_ok() -> Result<(), anyhow::Error> {
    ...
    let _response: HaneulTransactionBlockResponse = jsonrpc_client
        .request("haneul_getTransactionBlock", rpc_params![*tx_digest])
        .await
        .unwrap();
...
async fn test_get_objects_read() -> Result<(), anyhow::Error> {
    ...
    // Transfer some HANEUL to recipient
...
async fn test_pass_back_no_object() -> Result<(), anyhow::Error> {
    ...
    // TODO: this is publishing the wrong package - we should be publishing the one in `haneul-core/src/unit_tests/data` instead.
...
async fn test_access_old_object_pruned() {
    ...
    let effects = test_cluster
        .sign_and_execute_transaction(&tx_builder.transfer_haneul(None, sender).build())
        .await
        .effects;
...
async fn test_full_node_run_with_range_checkpoint() -> Result<(), anyhow::Error> {
    ...
    assert!(test_cluster.fullnode_handle.haneul_node.with(|node| {
        node.state()
