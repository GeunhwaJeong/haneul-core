...
async fn test_accumulators_root_created() {
    ...
    test_env.cluster.fullnode_handle.haneul_node.with(|node| {
...
async fn test_accumulators_disabled() {
    ...
    let tx = test_env
        .tx_builder(sender)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(1000, recipient)])
        .build();
    ...
    let tx = test_env
        .tx_builder(sender)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(1000, sender)])
        .build();
...
async fn test_deposits() {
    ...
    test_env.cluster.fullnode_handle.haneul_node.with(|node| {
        ...
        let accumulator_object =
            AccumulatorValue::load_object(child_object_resolver, None, recipient, &haneul_coin_type)
        ...
        input_consensus_objects.iter().find(|input_consensus_object| {
            matches!(input_consensus_object, InputConsensusObject::ReadOnly(obj_ref) if obj_ref.0 == HANEUL_ACCUMULATOR_ROOT_OBJECT_ID)
...
async fn test_multiple_settlement_txns() {
    ...
    let amounts_and_recipients = (0..20)
        .map(|_| (1u64, HaneulAddress::random_for_testing_only()))
        .collect::<Vec<_>>();

    let tx = test_env
        .tx_builder(sender)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), amounts_and_recipients.clone())
        .build();
    test_env.exec_tx_directly(tx).await.unwrap();
...
async fn test_withdraw_insufficient_balance() {
    ...
    let tx = test_env
        ...
        .transfer_haneul_to_address_balance(FundSource::coin(gas1), vec![(1000, sender)])
        .build();
    test_env.exec_tx_directly(tx).await.unwrap();
...
async fn test_sponsored_address_balance_storage_rebates() {
    ...
    let sender_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sender, &create_txn, Intent::haneul_transaction())
        .await
        .unwrap();
    let sponsor_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &create_txn, Intent::haneul_transaction())
        .await
        .unwrap();
    ...
    let sponsor_actual = test_env.get_haneul_balance(sponsor);
    let sender_actual = test_env.get_haneul_balance(sender);
    ...
    let sender_delete_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sender, &delete_txn, Intent::haneul_transaction())
        .await
        .unwrap();
    let sponsor_delete_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &delete_txn, Intent::haneul_transaction())
        .await
        .unwrap();
    ...
    let sponsor_final = test_env.get_haneul_balance(sponsor);
    let sender_final = test_env.get_haneul_balance(sender);
...
async fn setup_address_balance_account(
    ...
    let deposit_tx = test_env
        .tx_builder(sender)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(deposit_amount, sender)])
        .build();
...
fn create_address_balance_transaction_with_expiration(
    ...
    use haneul_types::transaction::{GasData, TransactionDataV1, TransactionExpiration};
...
async fn test_sponsor_insufficient_balance_charges_zero_gas() {
    ...
    let sender_sig1 = test_env
        ...
        .config
        .keystore
        .sign_secure(&sender, &tx1, Intent::haneul_transaction())
        .await
        .unwrap();
    let sponsor_sig1 = test_env
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &tx1, Intent::haneul_transaction())
        .await
        .unwrap();
    ...
    let sender_sig2 = test_env
        ...
        .config
        .keystore
        .sign_secure(&sender, &tx2, Intent::haneul_transaction())
        .await
        .unwrap();
    let sponsor_sig2 = test_env
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &tx2, Intent::haneul_transaction())
        .await
        .unwrap();
    ...
    let final_sponsor_balance = test_env.get_haneul_balance(sponsor);
    ...
    let final_sender_balance = test_env.get_haneul_balance(sender);
...
async fn test_soft_bundle_different_gas_payers() {
    ...
    let actual_balance1 = test_env.get_haneul_balance(sender1);
    let actual_balance2 = test_env.get_haneul_balance(sender2);
...
async fn test_multiple_deposits_merged_in_effects() {
    ...
    let initial_deposit = test_env
        .tx_builder(sender)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(10000, recipient)])
        .build();
    ...
    for amount in &withdraw_amounts {
        let withdraw_arg = haneul_types::transaction::FundsWithdrawalArg::balance_from_sender(
            *amount,
            haneul_types::gas_coin::GAS::type_tag(),
        );
        let withdraw_arg = builder.funds_withdrawal(withdraw_arg).unwrap();
    ...
    match &event.write.value {
        haneul_types::effects::AccumulatorValue::Integer(value) => {
            assert_eq!(
                *value, expected_net,
    ...
    match &event.write.operation {
        haneul_types::effects::AccumulatorOperation::Merge => {}
...
async fn test_address_balance_gas_budget_enforcement_with_storage() {
    ...
    assert!(
        matches!(
            error_kind,
            haneul_types::execution_status::ExecutionFailureStatus::InsufficientGas
        ),
...
async fn test_address_balance_large_rebate() {
    ...
    let initial_balance = test_env.get_haneul_balance(sender);
    ...
    let final_balance = test_env.get_haneul_balance(sender);
...
async fn test_sponsored_address_balance_storage_oog() {
    ...
    let sender_sig = test_env
        ...
        .config
        .keystore
        .sign_secure(&sender, &tx, Intent::haneul_transaction())
        .await
        .unwrap();
    let sponsor_sig = test_env
        .cluster
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &tx, Intent::haneul_transaction())
        .await
        .unwrap();
...
async fn test_get_all_balances() {
    ...
    let tx = test_env
        .tx_builder(sender)
        .transfer_haneul_to_address_balance(FundSource::coin(gas), vec![(1001, recipient)])
        .build();
...
async fn publish_and_mint_trusted_coin(test_env: &mut TestEnv, sender: HaneulAddress) {
...
async fn address_balance_stress_test() {
    ...
    for coin_type in &coin_types {
        let recipients: Vec<(u64, HaneulAddress)> =
            ...
            let handle = tokio::spawn(async move {
                ...
                while start_time.elapsed() < duration {
                    let (selected_types, recipients) = {
                        ...
                        let other_addresses: Vec<HaneulAddress> = all_addresses
                            .iter()
                            .filter(|a| **a != addr)
                            .copied()
                            .collect();
...
async fn test_simulate_address_funds_sufficient() {
    ...
    let tx = test_env
        .tx_builder(sender)
        .transfer_haneul_to_address_balance(FundSource::address_fund(), vec![(500, dbg_addr(2))])
        .build();
...
#[sim_test]
async fn test_address_balance_gas_pay_all_haneul() {
    let mut test_env = TestEnvBuilder::new()
        .with_proto_override_cb(Box::new(|_, mut cfg| {
            cfg.enable_address_balance_gas_payments_for_testing();
            cfg
