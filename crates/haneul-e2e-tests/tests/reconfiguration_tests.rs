...
async fn test_validator_candidate_pool_read() {
    ...
    let address: HaneulAddress = (&new_validator.account_key_pair.public()).into();
    let test_cluster = TestClusterBuilder::new()
...
async fn test_inactive_validator_pool_read() {
    ...
    let address = validator.with(|node| node.get_config().haneul_address());
    let staking_pool_id = test_cluster.fullnode_handle.haneul_node.with(|node| {
        node.state()
            .get_haneul_system_state_object_for_testing()
            .unwrap()
...
const VALIDATOR_STARTING_STAKE: u64 = 1_000_000_000_000_000; // 1M HANEUL
...
async fn test_reconfig_with_committee_change_basic() {
    ...
    let total_stake = test_cluster.fullnode_handle.haneul_node.with(|node| {
        node.state()
            .get_haneul_system_state_object_for_testing()
            .unwrap()
            .into_haneul_system_state_summary()
...
async fn test_epoch_flag_upgrade() {
    ...
    register_fail_point_arg("initial_epoch_flags", move || {
        ...
        let current_node = haneul_simulator::current_simnode_id();
...
async fn add_validator_candidate(
    ...
    let cur_validator_candidate_count = test_cluster.fullnode_handle.haneul_node.with(|node| {
        node.state()
            .get_haneul_system_state_object_for_testing()
            .unwrap()
            .into_haneul_system_state_summary()
    ...
    test_cluster.fullnode_handle.haneul_node.with(|node| {
        let system_state = node
            ...
            .get_haneul_system_state_object_for_testing()
            .unwrap();
        let system_state_summary = system_state.into_haneul_system_state_summary();
        assert_eq!(
...
async fn execute_remove_validator_tx(test_cluster: &TestCluster, handle: &HaneulNodeHandle) {
    let address = handle.with(|node| node.get_config().haneul_address());
    let gas = test_cluster
        .wallet
        .get_one_gas_object_owned_by_address(address)
        .await
...
async fn execute_add_stake_transaction(
    ...
    let tx = {
        ...
        let system_arg = ptb.obj(ObjectArg::HANEUL_SYSTEM_MUT).unwrap();
        ...
        stakes.into_iter().for_each(|(stake_for, stake_amount)| {
            ...
            ptb.command(Command::MoveCall(Box::new(ProgrammableMoveCall {
                ...
                module: "haneul_system".to_string(),
...
async fn execute_add_validator_transactions(
    ...
    let pending_active_count = test_cluster.fullnode_handle.haneul_node.with(|node| {
        let system_state = node
            .state()
            .get_haneul_system_state_object_for_testing()
            .unwrap();
    ...
    test_cluster.fullnode_handle.haneul_node.with(|node| {
        ...
        assert_eq!(
            pending_active_validators[pending_active_validators.len() - 1].haneul_address,
            address
        );
    });
}
