...
pub(crate) enum Error {
    ...
    Format(haneul_display::v2::FormatError),

    #[error("Path error: {0}")]
    Path(haneul_display::v2::FormatError),
...
impl MoveValue {
    ...
    async fn display(&self, ctx: &Context<'_>) -> Option<Result<Display, RpcError>> {
        async {
            ...
            for (field, value) in
                haneul_display::v1::Format::parse(limits.max_display_field_depth, &event.fields)
    ...
    async fn extract(
        ...
        async {
            let limits: &Limits = ctx.data()?;
            let extract = haneul_display::v2::Extract::parse(limits.display(), &path)
            ...
            let Some(haneul_display::v2::OwnedSlice {
                layout,
                bytes: native,
    ...
    async fn format(
        ...
        async {
            let limits: &Limits = ctx.data()?;
            let parsed = haneul_display::v2::Format::parse(limits.display(), &format)
...
#[async_trait]
impl<'f, 'r> haneul_display::v2::Store for DisplayStore<'f, 'r> {
    async fn object(
        &self,
        id: AccountAddress,
        ...
        Ok(Some(haneul_display::v2::OwnedSlice { layout, bytes }))
    }
}
...
fn format_error(
    wrap: impl FnOnce(haneul_display::v2::FormatError) -> Error,
    e: haneul_display::v2::FormatError,
) -> RpcError<Error> {
    use haneul_display::v2::FormatError as FE;
    match &e {
