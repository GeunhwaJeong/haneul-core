    ...
    async fn upload(&mut self, data_points: Vec<DataPoint>) -> anyhow::Result<TransactionEffects> {
        ...
        for data_point in &data_points {
            ...
            builder.command(Command::move_call(
                ...
                vec![
                    parse_haneul_type_tag(&format!("{package_id}::decimal_value::DecimalValue"))
                        .unwrap(),
                ],
                arguments,
            ));
    ...
    async fn execute(&mut self, tx: Transaction) -> anyhow::Result<ExecutedTransaction> {
        ...
        loop {
                ...
                .await
            {
                ...
                Err(err) => {
                    // jsonrpsee translate every HaneulError into jsonrpsee::core::Error, so we need to further distinguish
