...
async fn test_index_address_balance_accumulates() {
    ...
    cluster.send_haneul_to_address_balance(a, 100);
    cluster.send_haneul_to_address_balance(a, 200);
    cluster.send_haneul_to_address_balance(a, 300);

    // Send to B's address balance
    cluster.send_haneul_to_address_balance(b, 400);
    cluster.send_haneul_to_address_balance(b, 500);
...
async fn test_multiple_coin_types() {
    ...
    cluster.send_haneul_to_address_balance(recipient, 500);
    cluster.cluster.create_checkpoint().await;
    ...
    // Recipient should have both HANEUL and MY_COIN address balances
    assert_eq!(
        cluster
...
async fn test_snapshot_consistency() {
    ...
    cluster.send_haneul_to_address_balance(a, 100);
    cluster.send_haneul_to_address_balance(a, 200);
    cluster.send_haneul_to_address_balance(b, 300);
    cluster.cluster.create_checkpoint().await;
    ...
    cluster.send_haneul_to_address_balance(a, 400);
    cluster.send_haneul_to_address_balance(b, 500);
    cluster.cluster.create_checkpoint().await;
...
async fn test_address_to_address_transfer() {
    ...
    let (balances, _) = cluster
        ...
        .await
        .unwrap();
    // Single balance entry for HANEUL/ gas
...
async fn test_edge_cases() {
    ...
    request
        .metadata_mut()
        .insert("x-haneul-checkpoint", "10".parse().unwrap());
...
impl BalanceCluster {
    ...
    fn request_gas(&mut self, requester: HaneulAddress, amount: u64) -> ObjectRef {
    ...
    /// Publisher mints HANEUL to send to a recipient's address balance.
    fn send_haneul_to_address_balance(&mut self, recipient: HaneulAddress, amount: u64) {
    ...
    async fn batch_get_balances(
        &mut self,
        requests: Vec<(HaneulAddress, String)>,
        checkpoint: Option<u64>,
...
async fn paginate_list_balances(
    ...
    haneul_indexer_alt_consistent_api::proto::rpc::consistent::v1alpha::ListBalancesResponse,
    tonic::Status,
> {
