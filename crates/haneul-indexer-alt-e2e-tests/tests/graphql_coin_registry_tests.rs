...
use haneul_indexer_alt_e2e_tests::coin_registry::{self};
...
query GetObjects($owner: HaneulAddress!, $after: String) {
...
#[tokio::test]
async fn test_haneul() {
    // HANEUL coin is available from genesis, no need to publish
    let mut cluster = FullCluster::new().await.unwrap();
    cluster.create_checkpoint().await;

    let metadata = query_metadata(&cluster, "0x2::haneul::HANEUL").await;
...
async fn test_fixed_supply() {
    ...
    let currency = find::address_owned_by(&fx, HANEUL_COIN_REGISTRY_ADDRESS.into()).unwrap();
...
async fn query_owned_outputs(cluster: &FullCluster, owner: HaneulAddress) -> LegacyCoinOutputs {
...
async fn query_objects(cluster: &FullCluster, owner: HaneulAddress) -> Vec<(StructTag, ObjectRef)> {
    #[derive(Deserialize)]
    #[serde(rename_all = "camelCase")]
    struct PageInfo {
