...
async fn test_aggregation() {
    ...
    // Create multiple HANEUL coins for address A with different amounts
    create_coin(&mut cluster, a, 1);
    create_coin(&mut cluster, a, 2);
    create_coin(&mut cluster, a, 3);

    // Create HANEUL coins for address B
...
async fn test_multiple_coin_types() {
    ...
    let haneul_balance = DEFAULT_GAS_BUDGET as i64 - fx.gas_cost_summary().net_gas_usage();
    ...
    // P's balances should include HANEUL (left over from the gas coin) and MY_COIN (1000 + 200 + 30)
    // = 1230 from minting during publish and init.
    let mut balances = vec![
        (gas_type.clone(), haneul_balance as u64),
    ...
    assert_eq!(
        ...
        vec![
            ...
            (p.to_string(), gas_type.clone(), haneul_balance as u64),
        ]
    );
}
...
async fn test_transfers() {
    ...
    builder.transfer_haneul(b, Some(1000));
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.transfer_haneul(b, None);
...
/// `amount` to `owner`.
fn create_coin(cluster: &mut FullCluster, owner: HaneulAddress, amount: u64) -> ObjectRef {
