...
impl RpcExampleProvider {
    ...
    pub fn examples(&mut self) -> BTreeMap<String, Vec<ExamplePairing>> {
        [
            ...
            self.haneul_get_reference_gas_price(),
            self.haneulx_get_all_balances(),
            self.haneulx_get_all_coins(),
            self.haneulx_get_balance(),
            self.haneulx_get_coin_metadata(),
    ...
    fn batch_transaction_examples(&mut self) -> Examples {
        let signer = HaneulAddress::from(ObjectID::new(self.rng.r#gen()));
        let recipient = HaneulAddress::from(ObjectID::new(self.rng.r#gen()));
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    (
                        "sender_address",
                        json!(HaneulAddress::from(ObjectID::new(self.rng.r#gen()))),
                    ),
                    ("tx_bytes", json!(tx_bytes.tx_bytes)),
                    ("gas_price", json!(1000)),
                    ("epoch", json!(8888)),
    ...
    fn get_object_responses(&mut self, object_count: usize) -> Vec<HaneulObjectResponse> {
        (0..object_count)
            .map(|_| {
                let object_id = ObjectID::new(self.rng.r#gen());
    ...
    fn get_owned_objects(&mut self) -> Examples {
        let owner = HaneulAddress::from(ObjectID::new(self.rng.r#gen()));
        let result = (0..4)
    ...
    fn get_transfer_data_response(
        ...
        let object_change = ObjectChange::Transferred {
            ...
            object_type: parse_haneul_struct_tag("0x2::example::Object").unwrap(),
    ...
    fn get_events(&mut self) -> Examples {
        ...
        let event = HaneulEvent {
            ...
            sender: HaneulAddress::from(ObjectID::new(self.rng.r#gen())),
    ...
    fn haneulx_get_all_balances(&mut self) -> Examples {
        let address = HaneulAddress::from(ObjectID::new(self.rng.r#gen()));
    ...
    fn haneulx_get_all_coins(&mut self) -> Examples {
        let limit = 3;
        let owner = HaneulAddress::from(ObjectID::new(self.rng.r#gen()));
    ...
    fn haneulx_get_balance(&mut self) -> Examples {
        let owner = HaneulAddress::from(ObjectID::new(self.rng.r#gen()));
    ...
    fn haneul_get_latest_checkpoint_sequence_number(&mut self) -> Examples {
        let result = "507021";
        Examples::new(
    ...
    fn haneulx_get_coins(&mut self) -> Examples {
    ...
    fn haneulx_get_total_supply(&mut self) -> Examples {
    ...
    fn haneulx_get_validators_apy(&mut self) -> Examples {
        let result = vec![
            ValidatorApy {
                address: HaneulAddress::from(ObjectID::new(self.rng.r#gen())),
                apy: 0.06,
            },
            ValidatorApy {
                address: HaneulAddress::from(ObjectID::new(self.rng.r#gen())),
    ...
    fn haneulx_get_dynamic_field_object(&mut self) -> Examples {
        ...
        let struct_tag = parse_haneul_struct_tag("0x9::test::TestField").unwrap();
    ...
    fn haneulx_get_stakes(&mut self) -> Examples {
        ...
        let result = vec![
            DelegatedStake {
                validator_address: HaneulAddress::from(ObjectID::new(self.rng.r#gen())),
                staking_pool: ObjectID::new(self.rng.r#gen()),
                stakes: vec![
                    Stake {
                        staked_haneul_id: ObjectID::new(self.rng.r#gen()),
