...
impl BenchmarkBank {
    ...
    pub async fn generate(
        ...
        for chunk in chunked_coin_configs {
            let gas_coins = self.pay_haneul(chunk, &mut init_coin, gas_price).await?;
    ...
    async fn pay_haneul(
        ...
        let tx = make_pay_haneul_transaction(
            init_coin.0,
            vec![],
            recipient_addresses,
            amounts,
        ...
        let address_map: HashMap<HaneulAddress, Arc<AccountKeyPair>> = coin_configs
            .iter()
