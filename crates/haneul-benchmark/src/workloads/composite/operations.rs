...
impl Operation for AddressBalanceOverdraw {
    ...
    fn apply(
        ...
        let withdraw_amount = if account_state.haneul_balance == 0 {
            // reservations of zero are invalid, so the transaction will be rejected.
            0
        } else {
            // withdraw at least half the balance
            let half_balance = std::cmp::max(1, account_state.haneul_balance / 2);
            get_rng().gen_range(half_balance..=account_state.haneul_balance)
        };
