...
mod test {
    ...
    use haneul_benchmark::workloads::adversarial::AdversarialPayloadCfg;
    use haneul_benchmark::workloads::benchmark_move_base_dir;
    use haneul_benchmark::workloads::composite::CompositeWorkloadConfig;
    use haneul_benchmark::workloads::expected_failure::ExpectedFailurePayloadCfg;
    use haneul_benchmark::workloads::workload::ExpectedFailureType;
    use haneul_benchmark::workloads::workload_configuration::{
    ...
    use haneul_simulator::{SimConfig, configs::*};
    ...
    fn get_keep_alive_nodes(cluster: &TestCluster) -> HashSet<haneul_simulator::task::NodeId> {
        ...
        keep_alive_nodes.insert(haneul_simulator::current_simnode_id());
        keep_alive_nodes
    }

    fn handle_failpoint(
        dead_validator: Arc<Mutex<Option<DeadValidator>>>,
        keep_alive_nodes: HashSet<haneul_simulator::task::NodeId>,
        ...
        if rng.gen_range(0.0..1.0) < probability {
            ...
            *dead_validator = Some(DeadValidator {
            ...
            haneul_simulator::task::kill_current_node(Some(restart_after));
        }
    }
    ...
    async fn test_simulated_load_reconfig_with_prune_and_compact() {
        ...
        let node_state = test_cluster.fullnode_handle.haneul_node.clone().state();
    ...
    async fn test_data_ingestion_pipeline() {
        ...
        let _checkpoint: haneul_types::full_checkpoint_content::Checkpoint = (&proto_checkpoint)
            .try_into()
    ...
    async fn test_protocol_upgrade_compatibility_impl() {
        ...
        let manifest = haneul_framework_snapshot::load_bytecode_snapshot_manifest();
        ...
        let init_framework =
            haneul_framework_snapshot::load_bytecode_snapshot(starting_version).unwrap();
        let test_cluster = Arc::new(
        ...
        let _handle = tokio::task::spawn(async move {
            ...
            for version in starting_version..=max_ver {
                ...
                let new_framework = haneul_framework_snapshot::load_bytecode_snapshot(next_version);
    ...
    fn handle_bool_failpoint(
        eligible_nodes: &HashSet<haneul_simulator::task::NodeId>, // only given eligible nodes may fail
        probability: f64,
    ) -> bool {
        if !eligible_nodes.contains(&haneul_simulator::current_simnode_id()) {
    ...
    impl SimulatedLoadConfig {
        fn composite_only(
            config: haneul_benchmark::workloads::composite::CompositeWorkloadConfig,
        ) -> Self {
            Self {
                composite_weight: 1,
        ...
        Fut: std::future::Future<Output = ()> + Send,
    {
        ...
        if enable_surfer {
            let surfer_task = tokio::spawn(async move {
                // now do a haneul-surfer test
                let mut test_packages_dir = benchmark_move_base_dir();
                test_packages_dir.extend(["..", "..", "crates", "haneul-surfer", "tests"]);
                ...
                .await;
                info!("haneul_surfer test complete with results: {results:?}");
                ...
                move |_cluster: Arc<TestCluster>| async move {
                    ...
                    register_fail_point_arg("kill_checkpoint_fork_node", {
                        let forked_validators: Arc<
                            Mutex<HashSet<haneul_types::crypto::AuthorityPublicKeyBytes>>,
                        > = forked_validators.clone();
                        let checkpoint_overrides = checkpoint_overrides.clone();
                        let node_to_authority_map = node_to_authority_map.clone();
                        move || {
                            let current_node_id = haneul_simulator::current_simnode_id();
                            let authority_name =
                        ...
                        move || {
                            forked_validators.lock().unwrap().contains(
                                node_to_authority_map
                                    .get(&haneul_simulator::current_simnode_id())
                                    .unwrap(),
                            )
                        }
                    });
    ...
    async fn test_composite_workload() {
        ...
        let metrics = Arc::new(Mutex::new(
            haneul_benchmark::workloads::composite::CompositionMetrics::new(),
        ));

        use haneul_benchmark::workloads::composite::*;
    ...
    async fn test_simulated_load_async_post_processing_consistency() {
        ...
        let async_fullnode = test_cluster
            ...
            .await;
        let async_fn_name = async_fullnode.haneul_node.state().name;
        let async_fn_sim_id = async_fullnode.haneul_node.with(|n| n.get_sim_node_id());
        drop(async_fullnode);
        ...
        let sync_fullnode = test_cluster
            ...
            .await;
        let sync_fn_name = sync_fullnode.haneul_node.state().name;
        drop(sync_fullnode);
