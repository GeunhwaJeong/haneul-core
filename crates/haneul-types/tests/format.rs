...
fn get_registry() -> Result<Registry> {
    ...
    let kp1: HaneulKeyPair =
        HaneulKeyPair::Ed25519(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp2: HaneulKeyPair =
        HaneulKeyPair::Secp256k1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp3: HaneulKeyPair =
        HaneulKeyPair::Secp256r1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    ...
    let struct_tag = StructTag::from_str("0x2::coin::Coin<0x2::haneul::HANEUL>").unwrap();
    ...
    tracer
        .trace_type::<base_types::HaneulAddress>(&samples)
        .unwrap();
...
fn test_format() {
    ...
    insta::assert_yaml_snapshot!("haneul.yaml", registry);
}
