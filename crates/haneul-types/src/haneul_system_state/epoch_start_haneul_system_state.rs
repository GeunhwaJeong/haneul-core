...
use crate::base_types::{AuthorityName, EpochId, HaneulAddress};
...
pub trait EpochStartSystemStateTrait {
    ...
    fn get_validator_addresses(&self) -> Vec<HaneulAddress>;
    fn get_haneul_committee(&self) -> Committee;
    fn get_haneul_committee_with_network_metadata(&self) -> CommitteeWithNetworkMetadata;
...
/// This type captures the minimum amount of information from HaneulSystemState needed by a validator
/// to run the protocol. This allows us to decouple from the actual HaneulSystemState type, and hence
/// do not need to evolve it when we upgrade the HaneulSystemState type.
...
impl EpochStartSystemStateTrait for EpochStartSystemStateV1 {
    ...
    fn get_validator_addresses(&self) -> Vec<HaneulAddress> {
        self.active_validators
            .iter()
            .map(|validator| validator.haneul_address)
            .collect()
    }
            ...
            .map(|validator| {
                (
                    ...
                    (
                        ...
                        NetworkMetadata {
                            network_address: validator.haneul_net_address.clone(),
    ...
    fn get_consensus_committee(&self) -> ConsensusCommittee {
        ...
        // in the Haneul committee returned from get_haneul_committee().
        authorities.sort_by(|a1, a2| a1.authority_key.cmp(&a2.authority_key));

        for ((i, mysticeti_authority), haneul_authority_name) in authorities
            .iter()
            .enumerate()
            .zip(self.get_haneul_committee().names())
        {
            if haneul_authority_name.0 != mysticeti_authority.authority_key.to_bytes() {
                error!(
                    "Mismatched authority order between Haneul and Mysticeti! Index {}, Mysticeti authority {:?}\nHaneul authority name {}",
                    i, mysticeti_authority, haneul_authority_name
                );
            }
        }
...
pub struct EpochStartValidatorInfoV1 {
    ...
    pub haneul_net_address: Multiaddr,
...
mod test {
    ...
    fn test_haneul_and_mysticeti_committee_are_same() {
        ...
        assert_eq!(haneul_committee.num_members(), consensus_committee.size());
        assert_eq!(
