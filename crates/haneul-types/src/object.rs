...
impl MoveObject {
    ...
    pub fn get_layout(&self, resolver: &impl GetModule) -> Result<MoveStructLayout, HaneulError> {
    ...
    pub fn to_move_struct(&self, layout: &MoveStructLayout) -> Result<MoveStruct, HaneulError> {
    ...
    pub fn get_total_haneul(
        ...
        if self.type_.is_gas_coin() {
            ...
            // It's a coin, but its not HANEUL
            Ok(0)
        } else if self.type_.is_haneul_balance_accumulator_field() {
            ...
            // anywhere, which is 10B for HANEUL.
            assert!(
                v.value <= u64::MAX as u128,
                "HANEUL balance cannot exceed u64::MAX"
            );
            Ok(v.value as u64)
        } else {
...
pub enum Owner {
    ...
    /// The object ID is converted to HaneulAddress as HaneulAddress is universal.
...
impl PartialEq<ObjectID> for Owner {
    fn eq(&self, other: &ObjectID) -> bool {
        let other_id: HaneulAddress = (*other).into();
        match self {
...
fn is_object_debug_verbose() -> bool {
    static HANEUL_OBJECT_DEBUG_VERBOSE: Lazy<bool> =
        Lazy::new(|| std::env::var("HANEUL_OBJECT_DEBUG_VERBOSE").is_ok());
    *HANEUL_OBJECT_DEBUG_VERBOSE
}
...
impl ObjectInner {
    ...
    pub fn get_single_owner(&self) -> Option<HaneulAddress> {
    ...
    // context: https://github.com/GeunhwaJeong/haneul/pull/10679#discussion_r1165877816
    ...
    pub fn transfer(&mut self, new_owner: HaneulAddress) {
    ...
    pub fn get_layout(
        &self,
        resolver: &impl GetModule,
    ) -> Result<Option<MoveStructLayout>, HaneulError> {
        match &self.data {
    ...
    pub fn get_move_template_type(&self) -> HaneulResult<TypeTag> {
        let move_struct = self
            .data
            .struct_tag()
            .ok_or_else(|| HaneulErrorKind::TypeError {
...
impl Object {
    ...
    pub fn with_id_owner_gas_for_testing(id: ObjectID, owner: HaneulAddress, gas: u64) -> Self {
        let data = Data::Move(MoveObject {
            type_: GasCoin::type_().into(),
            has_public_transfer: true,
            version: OBJECT_START_VERSION,
    ...
    pub fn with_owner_for_testing(owner: HaneulAddress) -> Self {
    ...
    pub fn new_gas_with_balance_and_owner_for_testing(value: u64, owner: HaneulAddress) -> Self {
...
mod tests {
    ...
    fn test_object_digest_and_serialized_format() {
        ...
        let o = Object::new_move(
            g,
            Owner::AddressOwner(HaneulAddress::ZERO),
