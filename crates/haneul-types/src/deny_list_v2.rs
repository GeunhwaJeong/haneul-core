...
use crate::base_types::{EpochId, HaneulAddress};
...
impl ConfigKey {
    pub fn type_() -> StructTag {
        StructTag {
            address: HANEUL_FRAMEWORK_PACKAGE_ID.into(),
...
#[derive(Debug, Serialize, Deserialize, Clone)]
struct AddressKey(HaneulAddress);
...
fn check_new_regulated_coin_owners(
    new_regulated_coin_owners: BTreeMap<String, (Config, BTreeSet<HaneulAddress>)>,
    cur_epoch: EpochId,
    object_store: &dyn ObjectStore,
...
pub fn get_per_type_coin_deny_list_v2(
    ...
    let config: Config =
        get_dynamic_field_from_store(object_store, HANEUL_DENY_LIST_OBJECT_ID, &config_key).ok()?;
    Some(config)
}
