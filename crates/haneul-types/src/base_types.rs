...
impl MoveObjectType {
    ...
    pub fn staked_haneul() -> Self {
        Self(MoveObjectType_::StakedHaneul)
    }

    pub fn address(&self) -> AccountAddress {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => HANEUL_FRAMEWORK_ADDRESS,
            MoveObjectType_::StakedHaneul => HANEUL_SYSTEM_ADDRESS,
            MoveObjectType_::HaneulBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_) => HANEUL_FRAMEWORK_ADDRESS,
    ...
    pub fn module(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedHaneul => STAKING_POOL_MODULE_NAME,
            MoveObjectType_::HaneulBalanceAccumulatorField
    ...
    pub fn name(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedHaneul => STAKED_HANEUL_STRUCT_NAME,
            MoveObjectType_::HaneulBalanceAccumulatorField
    ...
    pub fn type_params(&self) -> Vec<Cow<'_, TypeTag>> {
        match &self.0 {
            ...
            MoveObjectType_::StakedHaneul => vec![],
    ...
    pub fn coin_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            ...
            MoveObjectType_::StakedHaneul => None,
            MoveObjectType_::HaneulBalanceAccumulatorField => None,
    ...
    pub fn balance_accumulator_field_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            MoveObjectType_::HaneulBalanceAccumulatorField => Some(GAS::type_tag()),
    ...
    pub fn is_haneul_balance_accumulator_field(&self) -> bool {
        matches!(self.0, MoveObjectType_::HaneulBalanceAccumulatorField)
    }
    ...
    pub fn size_for_gas_metering(&self) -> usize {
        ...
        match &self.0 {
            ...
            MoveObjectType_::StakedHaneul => 1,
            MoveObjectType_::Coin(inner) => bcs::serialized_size(inner).unwrap() + 1,
            MoveObjectType_::HaneulBalanceAccumulatorField => 1,
            MoveObjectType_::BalanceAccumulatorField(inner) => {
    ...
    /// Return true if `self` is 0x2::coin::Coin<0x2::haneul::HANEUL>
    ...
    pub fn is_staked_haneul(&self) -> bool {
        match &self.0 {
            MoveObjectType_::StakedHaneul => true,
            MoveObjectType_::GasCoin
            | MoveObjectType_::Coin(_)
            | MoveObjectType_::HaneulBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_)
    ...
    pub fn is_upgrade_cap(&self) -> bool {
        self.address() == HANEUL_FRAMEWORK_ADDRESS
    ...
    pub fn is(&self, s: &StructTag) -> bool {
        match &self.0 {
            ...
            MoveObjectType_::StakedHaneul => StakedHaneul::is_staked_haneul(s),
...
impl From<StructTag> for MoveObjectType {
    fn from(mut s: StructTag) -> Self {
        ...
        } else if StakedHaneul::is_staked_haneul(&s) {
            MoveObjectType_::StakedHaneul
...
impl From<MoveObjectType> for StructTag {
    fn from(t: MoveObjectType) -> Self {
        match t.0 {
            ...
            MoveObjectType_::StakedHaneul => StakedHaneul::type_(),
...
impl FromStr for ObjectType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.to_lowercase() == PACKAGE {
            ...
            let tag = parse_haneul_struct_tag(s)?;
...
pub const HANEUL_ADDRESS_LENGTH: usize = ObjectID::LENGTH;
...
#[cfg_attr(feature = "fuzzing", derive(proptest_derive::Arbitrary))]
pub struct HaneulAddress(
    #[schemars(with = "Hex")]
    #[serde_as(as = "Readable<Hex, _>")]
    [u8; HANEUL_ADDRESS_LENGTH],
);

impl HaneulAddress {
    pub const ZERO: Self = Self([0u8; HANEUL_ADDRESS_LENGTH]);
    ...
    pub fn generate<R: rand::RngCore + rand::CryptoRng>(mut rng: R) -> Self {
        let buf: [u8; HANEUL_ADDRESS_LENGTH] = rng.r#gen();
        Self(buf)
    }
    ...
    /// Return the underlying byte array of a HaneulAddress.
    ...
    pub fn try_from_padded(inputs: &ZkLoginInputs) -> HaneulResult<Self> {
        Ok((&PublicKey::from_zklogin_inputs(inputs)?).into())
    }

    /// Define as iss_bytes_len || iss_bytes || unpadded_32_byte_address_seed.
    pub fn try_from_unpadded(inputs: &ZkLoginInputs) -> HaneulResult<Self> {
...
impl TryFrom<&[u8]> for HaneulAddress {
    type Error = HaneulError;

    /// Tries to convert the provided byte array into a HaneulAddress.
...
impl TryFrom<Vec<u8>> for HaneulAddress {
    ...
    /// Tries to convert the provided byte buffer into a HaneulAddress.
    fn try_from(bytes: Vec<u8>) -> Result<Self, HaneulError> {
        Self::from_bytes(bytes)
    }
}

impl AsRef<[u8]> for HaneulAddress {
    fn as_ref(&self) -> &[u8] {
        &self.0[..]
    }
}
...
impl TryFrom<&ZkLoginAuthenticator> for HaneulAddress {
    ...
    fn try_from(authenticator: &ZkLoginAuthenticator) -> HaneulResult<Self> {
        HaneulAddress::try_from_unpadded(&authenticator.inputs)
    }
}
...
impl fmt::Display for HaneulAddress {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "0x{}", Hex::encode(self.0))
    }
}

impl fmt::Debug for HaneulAddress {
...
impl TxContext {
    ...
    pub fn sponsor(&self) -> Option<HaneulAddress> {
        self.sponsor.map(HaneulAddress::from)
    }
...
impl ObjectID {
    ...
    pub fn is_clock(&self) -> bool {
        *self == HANEUL_CLOCK_OBJECT_ID
    }
}

impl From<HaneulAddress> for ObjectID {
    fn from(address: HaneulAddress) -> ObjectID {
...
impl fmt::Display for MoveObjectType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}",
            to_haneul_struct_tag_string(&s).map_err(fmt::Error::custom)?
        )
    }
}
