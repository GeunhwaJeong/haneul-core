...
PROTOCOL_CONFIG_PATH = "crates/haneul-protocol-config/src/lib.rs"
...
def extract_notes_for_pr(pr):
    ...
    variables = {"owner": "HaneulLabs", "name": "haneul", "number": int(pr)}
...
def fetch_release_notes_for_commits(commits):
    ...
    for start in range(0, len(commits), BATCH_SIZE):
        ...
        variables = {"owner": "HaneulLabs", "name": "haneul"}
...
def extract_protocol_version(commit):
    ...
    Assumes that it is being called from the root of the haneul repository."""
    for line in git(
        "show", f"{commit}:crates/haneul-protocol-config/src/lib.rs"
    ).splitlines():
...
def print_changelog(pr, log):
    if pr:
        print(f"https://github.com/GeunhwaJeong/haneul/pull/{pr}: {log}")
    else:
        print(log)
